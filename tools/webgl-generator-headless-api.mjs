...
if (!file) {
  process.stderr.write(`${JSON.stringify({ok: false, error: {code: "headless_cli_error", message: "用法：node tools/webgl-generator-headless-api.mjs <map.webfmg|map.json|map.json.gz> [method] [jsonArguments]"}})}\n`);
  process.exit(1);
}

try {
  const bytes = await readFile(file);
  const text = isCompressedMapDocumentFilename(file) ? gunzipSync(bytes).toString("utf8") : bytes.toString("utf8");
