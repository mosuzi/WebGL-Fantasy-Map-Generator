...
const outputDir = join(root, "work", political ? "task335-e-political-views-10000" : "task335-d-gpu-resident-views-10000");
...
try {
  ...
  const report = await page.evaluate(run, {verify, political});
  ...
  if (verify) {
    assert.equal(report.cells, 10004, `335-${political ? "E" : "D"} 固定 10k 地图规模漂移`);
    if (political) {
...
async function run({verify, political}) {
