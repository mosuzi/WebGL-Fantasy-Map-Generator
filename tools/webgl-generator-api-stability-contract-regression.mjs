...
assert.equal(Object.values(API_METHODS).reduce((sum, methods) => sum + methods.length, 0), 204, "公开方法基线发生变化");
assert.deepEqual(capabilities.stabilitySummary, {stable: 196, experimental: 7, deprecated: 1}, "稳定等级统计错误");

const metadataEntries = flattenMetadata(capabilities.methodMetadata);
assert.equal(metadataEntries.length, 204, "方法元数据数量错误");
...
assert.deepEqual(groupQualifiedMethodNames(CONFIRM_REQUIRED_METHODS).edit, ["height.rebuildBaseDerived", "height.rebuildDownstreamDerived", "economy.assignCells", "economy.rebuild", "states.merge", "states.split", "features.applyTopology"], "嵌套编辑确认分组丢失方法路径");
...
assert.equal((consoleApiSource.match(/\{stable: "draft", mutates:/g) || []).length, 204, "运行时原始副作用元数据数量发生变化");
