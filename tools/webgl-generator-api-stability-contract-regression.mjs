...
assert.equal(Object.values(API_METHODS).reduce((sum, methods) => sum + methods.length, 0), 289, "公开方法基线发生变化");
assert.deepEqual(capabilities.stabilitySummary, {stable: 281, experimental: 7, deprecated: 1}, "稳定等级统计错误");

const metadataEntries = flattenMetadata(capabilities.methodMetadata);
assert.equal(metadataEntries.length, 289, "方法元数据数量错误");
...
assert.equal((consoleApiSource.match(/\{stable: "draft", mutates:/g) || []).length, 289, "运行时原始副作用元数据数量发生变化");
