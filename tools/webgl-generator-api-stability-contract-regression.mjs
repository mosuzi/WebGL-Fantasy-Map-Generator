...
assert.equal(Object.values(API_METHODS).reduce((sum, methods) => sum + methods.length, 0), 165, "公开方法基线发生变化");
assert.deepEqual(capabilities.stabilitySummary, {stable: 157, experimental: 7, deprecated: 1}, "稳定等级统计错误");

const metadataEntries = flattenMetadata(capabilities.methodMetadata);
assert.equal(metadataEntries.length, 165, "方法元数据数量错误");
...
assert.deepEqual(groupQualifiedMethodNames(CONFIRM_REQUIRED_METHODS).edit, ["height.rebuildBaseDerived", "height.rebuildDownstreamDerived"], "嵌套编辑确认分组丢失方法路径");
...
assert.equal((consoleApiSource.match(/\{stable: "draft", mutates:/g) || []).length, 165, "运行时原始副作用元数据数量发生变化");
