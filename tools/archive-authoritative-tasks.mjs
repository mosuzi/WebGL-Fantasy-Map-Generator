...
function renderCurrentPlan(activeTasks, apiBaseline) {
  ...
  const activeOrder = activeTasks.map(task => task.number).join(" → ") || "空";
...
> **执行门禁（${statusDate}）**：当前未归档任务为${activeSummary}。${activeBoundary}当前固定顺序为 \`${activeOrder}\`。第 53 项已移除，第 278 项已由第 279 项取代，其余既有完成状态见归档索引。
