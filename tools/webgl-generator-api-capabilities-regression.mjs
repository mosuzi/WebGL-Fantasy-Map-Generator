...
async function inspectCapabilities(page, {cells, seed, template}) {
  return page.evaluate(async ({cells, seed, template}) => {
    ...
    const expectedConfirmGroups = {
      ...
      climate: ["applyDownstreamRebuild"],
      edit: ["height.rebuildBaseDerived", "height.rebuildDownstreamDerived", "economy.assignCells", "economy.rebuild", "states.merge", "states.split", "features.applyTopology"]
    };
    ...
    if (JSON.stringify(capabilities.stabilitySummary) !== JSON.stringify({stable: 194, experimental: 7, deprecated: 1})) failures.push("稳定等级统计不是 194 / 7 / 1");
