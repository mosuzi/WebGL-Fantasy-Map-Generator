...
function expandedDirectDefinitions(inventory) {
  ...
  return [
    ...
    direct("DM-05", "基础画布鼠标 / 触摸板滚轮导航", "canvas-navigation", ["canvas:map-canvas"], 1, {
      start: "wheel 按 ctrlKey、deltaMode、横向分量及像素步长自动分类，并锁定短 burst 意图", move: "鼠标缩放保持光标世界锚点；触摸板普通 wheel 消费双轴 delta 平移", complete: "每次事件更新相机并请求预览，短 burst 结束后按现有延时提交", clickNoMove: "零 delta 不产生相机变化", cancel: "事件本身无跨帧草稿，短 burst 超时自然失效", pointerCancel: "不适用", captureLost: "不使用指针捕获", conflicts: "活动编辑锁默认拦截 wheel；ctrlKey pinch 始终缩放", history: "相机变化不写地图历史", recovery: "新 burst 从当前相机与事件特征重新分类", refs: [[FILES.viewportInput, ["classifyViewportWheelIntent", "zoomCameraAtClientPoint", "wheelBurst", "panCamera(-delta.x, -delta.y"]], [FILES.runtime, ["\"pointercancel\", \"wheel\""]]]
    }),
