...
const inboundMap = structuredCloneMap(map);
const inboundLake = inboundMap.pack.features[outletLake.i];
const inboundRiver = findRiver(inboundMap, outletId);
const inletLakeCell = inboundMap.pack.cells.c[inboundRiver.cells[0]].find(cell => Number(inboundMap.pack.cells.f[cell]) === Number(inboundLake.i));
assert(Number.isInteger(inletLakeCell), "固定 seed 的出口岸格应与湖泊直接相邻");
inboundRiver.cells = [inboundRiver.cells[0], inletLakeCell];
assert.equal(inspectLakeOutletChange(inboundMap, inboundLake.i, outletId).code, "invalid-outlet-path", "岸格流入湖泊不得冒充自然出口");
