...
assert(JSON.stringify(HEIGHT_BASE_REBUILD_STEPS) === JSON.stringify(["features", "rivers", "states"]), `基础派生顺序异常：${HEIGHT_BASE_REBUILD_STEPS}`);
...
assert(JSON.stringify(successCalls) === JSON.stringify(["features", "rivers", "states"]), `完整基础派生调用顺序异常：${successCalls}`);
assert(success.steps.length === 3 && success.steps.every(step => step.executed), "完整基础派生步骤摘要异常");
assert(success.constraint.includes("Feature / 岸线 -> 河流 / 生物群系 / 道路 -> 国家 / 省份 / 城镇 / 道路"), "完整基础派生没有解释拓扑顺序");

const featureFailureCalls = [];
const featureFailure = rebuildHeightBaseDerived(kind => {
  featureFailureCalls.push(kind);
  return createRegenerationResult(kind, "未执行", "缺少 Feature 输入");
});
assert(featureFailure.executed === false, "Feature 失败没有返回 executed=false");
assert(JSON.stringify(featureFailureCalls) === JSON.stringify(["features"]), `Feature 失败后仍执行了下游：${featureFailureCalls}`);
assert(featureFailure.constraint.includes("高度基础派生在Feature 与岸线停止"), `Feature 失败摘要异常：${featureFailure.constraint}`);

const riverFailureCalls = [];
const riverFailure = rebuildHeightBaseDerived(kind => {
  riverFailureCalls.push(kind);
  return createRegenerationResult(kind, kind === "rivers" ? "未执行" : "features done", "缺少河流输入");
});
assert(riverFailure.executed === false, "河流失败没有返回 executed=false");
assert(JSON.stringify(riverFailureCalls) === JSON.stringify(["features", "rivers"]), `河流失败后仍执行了下游：${riverFailureCalls}`);
...
const stateFailure = rebuildHeightBaseDerived(kind => {
  ...
  return createRegenerationResult(kind, kind === "states" ? "暂未执行" : `${kind} done`, "constraints");
});
assert(stateFailure.executed === false, "国家失败没有返回 executed=false");
assert(JSON.stringify(stateFailureCalls) === JSON.stringify(["features", "rivers", "states"]), `国家失败调用顺序异常：${stateFailureCalls}`);
assert(stateFailure.steps[0].executed && stateFailure.steps[1].executed && !stateFailure.steps[2].executed, "部分完成步骤摘要异常");
...
console.log(JSON.stringify({
  ...
  featureFailure: {executed: featureFailure.executed, calls: featureFailureCalls, summary: featureFailure.constraint},
