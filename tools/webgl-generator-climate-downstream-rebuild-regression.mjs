...
assert.match(panelVueSource, /useDebugMode/);
assert.match(panelVueSource, />查看更新范围</);
assert.match(panelVueSource, /更新中…/);
assert.match(panelVueSource, /更新受气候影响的内容/);
assert.match(panelVueSource, /仍建议更新/);
const ordinaryTemplate = panelVueSource.slice(0, panelVueSource.indexOf('<details v-if="debugEnabled"'));
assert.doesNotMatch(ordinaryTemplate, /受约束下游重算|"stale"|"fresh"|requestedSystems|executionOrder/, "普通气候面板仍暴露开发术语或内部 key");
assert.match(panelVueSource, /<details v-if="debugEnabled" class="climate-downstream-diagnostics">/);
assert.match(panelVueSource, /<summary>开发诊断<\/summary>/);
assert.match(panelVueSource, /executionOrder: props\.state\.downstreamPreview/, "调试模式应保留依赖执行顺序");
