...
const manifest = {
  ...
  runtimeProfiles: ["browser", "headless-readonly"],
  entry: "docs/ai/README.md"
};
const files = {
  "manifest.json": manifest,
  "api-catalog.json": {manifest, browser: catalog, headless: headlessCatalog},
  "recipe-catalog.json": {schemaVersion: PLANNER_RECIPE_SCHEMA_VERSION, recipes},
  "domain-capability-map.json": {domains: domainRoutes},
  "api-catalog.md": renderMarkdown(catalog, headlessCatalog, recipes)
};
...
console.log(check ? `AI 文档机器目录一致：浏览器 ${catalog.length} 方法、无头 ${headlessCatalog.length} 方法、${recipes.length} 配方。` : `已生成 AI 文档机器目录：${outputDirectory}`);
...
function renderMarkdown(browser, headless, recipes) {
