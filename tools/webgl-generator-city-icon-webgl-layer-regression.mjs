...
assert.deepEqual(CITY_ICON_BASE_CSS_SIZE, {width: 10.5, height: 8}, "城镇图标基准盒漂移");
...
for (const sample of nameWidthCases) {
  ...
  const outlineWidths = scales.map(scale => {
    ...
    return extent * CITY_ICON_BASE_CSS_SIZE.height * size.factor + 1.8;
  });
...
assert.match(CITY_ICON_FRAGMENT_SHADER_SOURCE, /smoothstep\(0\.9 - antialiasWidth, 0\.9 \+ antialiasWidth, distancePx\)/, "深色外沿没有收细为对称细线");
assert.match(CITY_ICON_FRAGMENT_SHADER_SOURCE, /smoothstep\(0\.45 - antialiasWidth, 0\.45 \+ antialiasWidth, distancePx\)/, "白色线芯没有同步收细");
