...
正式应用已经跨过早期占位原型阶段。权威任务第 28～52、54～177 项已完成，第 53 项按用户决定移除；公开 API 当前覆盖 11 个命名空间、208 个方法和 110 个编辑方法，稳定等级为 `200 / 7 / 1`。第 94～100 项完成地图层级、省份命名、国家详情编辑、首都候选、政体弹框、省界短虚线和政治标签避让；第 101～107 项完成全功能交互审计，固定 `102` 个交互表面，统一浏览器主套件 `25 / 25`、视觉矩阵 `12 / 12`，形成 `29` 个问题和未批准整改候选 `Q-25～Q-33`；第 108～133 项完成本机字体与样式、面板、命名、玩家化文案、旧图兼容、海底、洋流与语义化悬停；第 134～161 项完成标签、洋流、旧图、军事 / 城镇视觉、共享边界拓扑实验室、文字清晰度和地图尺寸开关；第 162～164 项完成全局品牌加载页、全局工具组贴边收起和正式应用 / 全部 prototype 的 Vercel 单次统一部署；第 165～171 项完成贴边箭头几何居中、正式画卷加载页、拓扑实验室差异可视化、地图画布随窗口实时同步、独立画卷文字视觉概念稿、无高光古画方向及旧纸自卷画轴 / 阴刻小篆印章；第 172～174 项完成海岸形变诊断与沿岸对象保护、高度画笔玩家交互收敛，以及指定 PNG 阴刻印面的概念稿替换；第 175～177 项完成高度面板密度与衰减语义收敛、认可画轴迁入正式 APP，以及高度面板按钮留白修正。正式加载页现已使用旧纸自卷画轴和指定 PNG 印面，保留真实加载状态、错误状态与语义版本号；独立 prototype 继续作为视觉基线。当前权威任务清单为空，交互审计候选 `Q-25～Q-33` 没有随本批转正。批准范围统一查看 `docs/current-plan.md` 的“权威任务清单”；README、专题文档中的“下一步”只作候选或历史语义，不能覆盖权威清单。
...
然后只按 `docs/current-plan.md` 的活动权威任务继续。当前第 28～52、54～177 项已完成，权威任务清单为空；不得从历史里程碑、README、FOLLOWUPS 或专题文档的“下一步”自行创造任务，`Q-25～Q-33` 仍是未批准候选。第 174 项指定 PNG 印面的旧纸自卷画轴已在第 176 项迁入正式 APP，独立 prototype 继续作为视觉基线。
