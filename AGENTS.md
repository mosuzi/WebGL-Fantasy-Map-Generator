...
正式应用已经跨过早期占位原型阶段。权威任务第 28～44 项已完成，公开 API 当前覆盖 11 个命名空间、168 个方法和 78 个编辑方法；第 45～52、54 项已经整理但按用户 2026-07-15 最新指令暂停，先完成现有功能与界面信息架构审计；第 53 项按用户决定移除，短期不深化纹章系统。审计结论见 `docs/audits/ui-function-and-information-architecture-audit.md`。批准范围统一查看 `docs/current-plan.md` 的“权威任务清单”。在用户确认新的 UI 整改清单和优先级前，不得默认从第 45 项开工；README、专题文档中的“下一步”只作候选或历史语义，不能覆盖权威清单。
