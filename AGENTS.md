...
正式应用已经跨过早期占位原型阶段。权威任务第 28～52、54～124 项已完成，第 53 项按用户决定移除；公开 API 当前覆盖 11 个命名空间、208 个方法和 110 个编辑方法，稳定等级为 `200 / 7 / 1`。第 94～100 项完成地图层级、省份命名、国家详情编辑、首都候选、政体弹框、省界短虚线和政治标签避让；第 101～107 项完成全功能交互审计，固定 `102` 个交互表面，统一浏览器主套件 `25 / 25`、视觉矩阵 `12 / 12`，形成 `29` 个问题和未批准整改候选 `Q-25～Q-33`；第 108～124 项完成本机字体读取、样式滑动条单位布局、本机字体可读名称、标签效果细步长、控制面板 Tab 居中、次级面板关闭返回、生物群系专业汉化、国家级省份后缀策略、文化命名风格映射、跨海国家标签本土锚定、外交完整地图往返、气候派生更新玩家化、海岸编辑预检收起、玩家化高度升降 / 范围平滑、内置 / 自定义显示单位与完整地图往返、单位页重复厘米换算移除，以及样式斜体紧凑热区。用户新批准的生成、样式、交互、导出、地形与海洋任务已按依赖冻结为第 110～133 项，当前只执行第 125 项；每项调查、开发、静态审查后本地提交，中途不启动浏览器、不推送，全部代码完成后统一浏览器验收并推送。交互审计候选 `Q-25～Q-33` 没有随本批转正。批准范围统一查看 `docs/current-plan.md` 的“权威任务清单”；README、专题文档中的“下一步”只作候选或历史语义，不能覆盖权威清单。
...
然后只按 `docs/current-plan.md` 的活动权威任务继续。当前活动批次为第 110～133 项，第 125 项优先；不得从历史里程碑、README、FOLLOWUPS 或专题文档的“下一步”自行创造任务。每项完成后本地提交但不启动浏览器、不推送，第 133 项完成后才统一浏览器验证和推送；`Q-25～Q-33` 仍是未批准候选。
