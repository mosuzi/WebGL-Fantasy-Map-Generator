...
2026-07-30 用户直接修正已把控制面板顶部七个 Tab 移出滚动区，只有 `.control-panel-tab-panels` 承担正文滚动；debug 模式“开发模式”按钮保持内部单行，窄屏时仍可作为整体随工具栏换行。系统 Chrome 三档视口测得父区不滚、Tab 位移 `0px`、正文可到底、document 溢出为 `0`，七 Tab、键盘与开发入口正常，两路独立复核为 `ACCEPT`。
