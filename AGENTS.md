...
正式应用已经跨过早期占位原型阶段。权威任务第 28～52、54～198 项已完成，第 53 项按用户决定移除；公开 API 当前覆盖 11 个命名空间、208 个方法和 110 个编辑方法，稳定等级为 `200 / 7 / 1`。第 94～100 项完成地图层级、省份命名、国家详情编辑、首都候选、政体弹框、省界短虚线和政治标签避让；第 101～107 项完成全功能交互审计，固定 `102` 个交互表面，统一浏览器主套件 `25 / 25`、视觉矩阵 `12 / 12`，形成 `29` 个问题和未批准整改候选 `Q-25～Q-33`；第 108～133 项完成本机字体与样式、面板、命名、玩家化文案、旧图兼容、海底、洋流与语义化悬停；第 134～161 项完成标签、洋流、旧图、军事 / 城镇视觉、共享边界拓扑实验室、文字清晰度和地图尺寸开关；第 162～164 项完成全局品牌加载页、全局工具组贴边收起和正式应用 / 全部 prototype 的 Vercel 单次统一部署；第 165～171 项完成贴边箭头几何居中、正式画卷加载页、拓扑实验室差异可视化、地图画布随窗口实时同步、独立画卷文字视觉概念稿、无高光古画方向及旧纸自卷画轴 / 阴刻小篆印章；第 172～174 项完成海岸形变诊断与沿岸对象保护、高度画笔玩家交互收敛，以及指定 PNG 阴刻印面的概念稿替换；第 175～177 项完成高度面板密度与衰减语义收敛、认可画轴迁入正式 APP，以及高度面板按钮留白修正；第 178 项把纸面、纸卷与进度收敛为 `transform / opacity` 合成动画，消除正式画卷展开的可见卡顿；第 179 项增加 `2500ms` 最短可见门槛，确保快速 ready 也能完整播放全部有限画卷动画；第 180～184 项完成高度派生更新入口常驻、可选海底影响、最小半径下调、屏幕正圆命中契约、受控采样节奏和深绿至红色的细分高度色带；第 185 项进一步将默认高度梯度收敛为低饱和度的墨绿至暗绛红配色，第 186 项将其扩展为十八个手工停靠色；第 187 项修复常驻更新按钮与高度编辑启停按钮的零间距，第 188 项收紧“影响海底”开关的整行热区；第 189～190 项新增整笔冻结首个落点高度的等高刷子，以及默认归零且与升降强度隔离的稳定扰动控制；第 191～193 项完成政体与合法国号后缀独立选择、所有画笔取消投影椭圆补偿并统一正圆，以及省份按国家、城镇按国家 / 省份局部重设；第 194 项修复真实旧存档下国家创建的运行时缓存、稀疏高 ID 扩容和失败原子性；第 195 项完成 Grid Cells 诊断图层与面向 AI 的可判定 API 设计，但没有授权实施；第 196～197 项完成政体弹框可读布局和全部对象列表默认列宽审计，只收敛国家 / 政体列表的首都列；第 198 项让道路与河流在平移、缩放 dirty 预览帧中复用旧网格并随相机仿射变换，空闲后仍按原路径精确重建。正式加载页继续使用旧纸自卷画轴和指定 PNG 印面，保留真实加载状态、错误状态与语义版本号；独立 prototype 继续作为视觉基线。当前权威任务清单为空，交互审计候选 `Q-25～Q-33` 和第 195 项的实施阶段没有随本批转正。批准范围统一查看 `docs/current-plan.md` 的“权威任务清单”；README、专题文档中的“下一步”只作候选或历史语义，不能覆盖权威清单。
...
然后只按 `docs/current-plan.md` 的活动权威任务继续。当前第 28～52、54～198 项已完成，权威任务清单为空；不得从历史里程碑、README、FOLLOWUPS 或专题文档的“下一步”自行创造任务，`Q-25～Q-33` 仍是未批准候选。第 174 项指定 PNG 印面的旧纸自卷画轴已在第 176 项迁入正式 APP，第 178 项改为合成友好的展开路径，第 179 项保证快速 ready 时完整播放；第 180～188 项完成本轮高度编辑、默认色带、面板间距和开关热区收敛，第 189～190 项完成等高刷子与稳定扰动，第 191～193 项完成政体 / 国号、正圆画笔与政治范围局部重设，第 194 项完成真实旧存档国家创建兼容，第 195 项只完成 Cells / AI API 详细设计且实施阶段未获批准，第 196～197 项完成政体弹框与对象列表列宽收敛，第 198 项完成道路 / 河流视口预览复用试验但尚未提交；独立 prototype 继续作为视觉基线。
