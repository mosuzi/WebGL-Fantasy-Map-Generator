...
正式应用已经跨过早期占位原型阶段。权威任务第 28～44、55～56 项已完成，公开 API 当前覆盖 11 个命名空间、168 个方法和 78 个编辑方法；第 45～52、54 项已经整理但保持后置；第 53 项按用户决定移除，短期不深化纹章系统。现有功能与界面信息架构审计已经由用户批准提升为权威任务第 55～60 项，审计证据见 `docs/audits/ui-function-and-information-architecture-audit.md`。批准范围统一查看 `docs/current-plan.md` 的“权威任务清单”：现在从第 57 项继续 UI 整改，完成第 57～60 项后再恢复第 45～52、54 项；README、专题文档中的“下一步”只作候选或历史语义，不能覆盖权威清单。
