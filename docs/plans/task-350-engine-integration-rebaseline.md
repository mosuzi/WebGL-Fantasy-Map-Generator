...
> 历史状态：2026-08-20 经用户批准，替代“继续用完整浏览器门逐个发现基础缺口”的执行方式；第 350 项现已完成并归档，相关成果随第 349～354 项进入 `main`。本文只保存当时重新基线与整体修补方案，不是当前执行计划。
>
> 历史执行分支：`codex/map-core-engine-architecture-plan`。成文时的“不合入 `main`”约束已由后续用户批准和完成合入取代；主线程唯一写者及逐阶段 `ACCEPT` 规则只描述当时执行流程。
>
