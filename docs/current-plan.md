...
13. 控制面板 tab 化第一刀已完成：固定侧栏只保留“控制面板”入口；`generation-panel` 已合并生成配置、专题选择器、图层选择和管理页面入口，并以 `生成 / 专题 / 图层 / 管理` 四个 tab 呈现；图层开关已改为带选中高亮的按钮样式，专题、图层显隐、“高度专题显示海底”和城市标签上限会同步到浏览器 `localStorage` 并在下次打开时恢复；城市标签上限滑动条会按当前地图城市总数动态设置最大值，默认候选上限为全量城市，避免大图被固定候选上限截断。
