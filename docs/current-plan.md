...
2026-07-14 执行补充：权威任务第 15～22 项采用代码门禁快速迭代，每项只跑语法检查、专项回归、生产构建和差异检查，不逐项启动浏览器；真实浏览器验证集中到第 23 项“编辑器基础设施综合浏览器验收”。每项仍单独提交，全部完成前不推送。
...
2026-07-15 追加计划：用户新增权威任务第 26 项“外交关系与交战区一致性”。当前 `createWarzoneZone()` 会优先读取 `pack.military.fronts`，而手动把战争关系改为中立或更友好关系时，旧 campaign / front 可能继续被地区生成读取；地区生成也没有对 front 双方的当前外交关系做最终校验。第 26 项是独立开放项，只修交战区一致性，不扩展其它地区类型或动态战争系统。

2026-07-15 追加计划：用户新增权威任务第 27 项“列表选中项滚动居中一致性”。城市管理等面板当前虽然会把选中行带入可见范围，但真实表现没有稳定落在滚动视口中央；历史上 `UiObjectTable` 已实现过居中算法，因此本项按回归修复处理，并复查虚拟表格、普通表格、文化 / 宗教树状总览、外交矩阵和其它存在“选中项 + 独立滚动视口”的路径。第 27 项只统一选中滚动契约，不改对象选择、地图定位或面板布局语义。

2026-07-15 实施收口：权威任务第 25～27 项均已达到代码验收。军事重生成已接入控制面板并补齐 UI / API 同路径结果摘要；战争派生一致性已覆盖外交编辑、军事生成、Warzone 生成、旧地图加载和 GeoJSON；列表居中已统一普通 / 虚拟公共表格、文化 / 宗教树状总览和外交矩阵。按用户最新“先快速迭代、代码跑测试即可”指令，本轮不启动浏览器，保留纯代码专项、既有回归、生产构建和差异检查结果。
...
7. 军事重新生成。`权威任务第 25 项，已完成`
   ...
   - 完成记录：控制面板已增加“军事”重生成入口；运行时会拒绝缺少有效国家的地图，返回扰动编号、军团 / 战线 / 战役前后数量和 affected 摘要，并保持 `map.military === pack.military`、军事 fresh、地区 stale。各国有效 `unitRatios` 会保留；旧全局战报保留为带 `archived / archiveReason / archiveGeneration` 的静态档案，军事面板不会把它们按旧 id 自动挂到新军团。`regress:military-regeneration` 的 3000 cells 固定样本通过，军团 `56 -> 56`、战线 `2 -> 2`、战役 `1 -> 1`，affected 共 `57` 项。
