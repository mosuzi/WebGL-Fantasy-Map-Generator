...
当前已提交并推送的 API 基线是 `257 / 135`；第 207 项步骤 3 的本地工作区基线已扩展为：`window.webglGeneratorApi` 覆盖 `15` 个命名空间、`269` 个公开方法和 `147` 个编辑方法，稳定等级为 `261 / 7 / 1`；`269 / 269` 方法可通过 `info.describe` 发现，`objects` 覆盖 `20` 类对象，`cells` 已提供八个读取、定位、扫描与动作预检方法。本批全部统一验收后再推送该工作区基线。
