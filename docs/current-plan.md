...
2026-07-16 用户补充三个国家生命周期问题，冻结为第 61～63 项：首都省份创建保护、新建国家疆域着色扩张、删除国家档案彻底移除。三项已按编号逐项达到最小验收并独立提交，专项回归、生产构建和同一 Chrome 内的集中交互验收均已完成。
...
> **执行门禁（2026-07-16）**：第 45～52、54～63 项已完成；本批次集中验证已通过，当前没有活动权威任务。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、186 个公开方法和 90 个编辑方法，稳定等级为 `178 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。

继续采用目标模式：只实现第 61～63 项的封闭范围，达到当前项最小验收后立即提交并转入下一项；不影响当前验收的新发现只写入 `FOLLOWUPS.md`。

- **权威任务第 61 项：禁止在既有国家首都所在省份创建国家。** `已完成；来源：用户实测`
  - 范围：国家新增命令在 UI 与 API 共用的 seed 校验中识别目标 cell 所属省份；只要该省份包含任一未删除国家的首都，就拒绝创建。其它陆地 cell 的既有创建语义保持不变。
  - 验收：首都所在省份内选择首都 cell 或其它 cell 均不产生国家、省份、城市或历史变化，并返回明确拒绝信息；非首都省份或允许区域仍可创建，撤销 / 重做保持完整。
  - 完成记录：新增共享 `inspectStateCreation()`；画布新增、API 新增和命令 `apply` 共用首都省份保护。固定地图的首都 cell `578` 与同省 cell `644` 均返回 `capital-province-protected`，完整政治 / 城市 / cell 快照与历史保持不变；允许 cell `547` 创建国家 #18，并完成撤销 / 重做。Chrome 实测点击魏共和国首都所在省份后国家列表仍为 21 条、撤销保持禁用，并显示明确拒绝信息。

- **权威任务第 62 项：新建国家可立即着色扩展疆域。** `已完成；来源：用户实测`
  - 范围：修正国家笔刷正常松手与取消事件的提交 / 回滚语义；新建完成后继续把新国家设为列表目标和地图选择对象，用户进入国家编辑即可用该国颜色扩张疆域。
  - 验收：正常 `pointerup` 形成一条国家笔刷历史并保留扩张结果，`pointercancel` 恢复预览且不写历史；新建国家作为目标完成一次扩张后 grid / pack 归属、颜色渲染、国家统计及撤销 / 重做一致。
  - 完成记录：国家笔刷的正常 `pointerup` 已接回 `finishStateStroke()`，`pointercancel` 改为只回滚预览。固定地图新建国家 #18 后扩张 grid cell `682` 与 pack cells `96 / 97`，国家统计增加且只写入一条历史；撤销 / 重做恢复一致。静态门禁确认新国家仍同时进入面板目标与地图 selection。Chrome 新建“北海王国”后立即进入国家编辑，单次刷涂使影响格 `6 -> 7`、人口 `15.7 万 -> 31.2 万`。

- **权威任务第 63 项：删除国家后彻底移除国家档案。** `已完成；来源：用户实测`
  - 范围：国家删除命令在清空疆域、城市与省份归属后，从 `politics.states` 和独立 `pack.states` 对应槽位移除档案；国家列表、目标、对象索引和导出不再保留零人口幽灵国家，不重排其它国家 ID。
  - 验收：删除后对应 state 槽位为空、国家列表无该项、关联 cell / 城市 / 省份已清理；撤销完整恢复档案与关联数据，重做再次移除，保存 / 导出与既有国家编辑回归通过。
  - 完成记录：国家删除不再保留 `removed` 档案，而是在不重排 ID 的前提下把 `politics.states[id]` 与独立 `pack.states[id]` 置空；国家面板桥接层、Vue 列表与对象解析器兼容过滤旧 tombstone。国家 #18 删除后 grid / pack / 城市无归属残留、原省份移除、对象解析及完整地图 / GeoJSON 导出均无该国；撤销完整恢复，重做再次移除。Chrome 删除刚创建的“北海王国”后列表 `22 -> 21`，该名称和 id 均不再出现，目标回到中立。

当前权威任务清单为空。第 45～52、54～63 项已完成并移出活动清单；第 53 项已移除且不作为隐含待办。本批次集中验证已完成，不自动创建后续工作。
