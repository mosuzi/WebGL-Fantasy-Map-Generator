...
- 编辑命令 `domain` 军事批次已完成：军事编辑命令 9 个导出工厂均已主动声明 `domain: "military"`；至此当前 `*-edit-commands.js` 主要命令域已覆盖，后续重点转向残留直接历史路径和更精确的 `effects.affected`。刷子与 GEO 导入命令已补充 `derived-system#height-brush/state-brush/province-brush/geo-import` 来源，保留原对象或 grid cells 目标。
