...
> **执行门禁（2026-08-23）**：第 351 项“修复 100k 视图切换失败并完成全视图 GPU 快切与后台预热”已获用户明确批准，唯一任务分支为 `codex/task-351-view-switch-repair`。该分支建立在未合入 `main` 的第 349～350 项架构分支之上，仍不得合入 `main`；只有第 351 项全部验收通过后才允许推送任务分支。当前固定顺序为 `351-0 → 351-1 → 351-2 → 351-3`。
...
### 权威任务第 351 项：修复 100k 视图切换失败并完成全视图 GPU 快切与后台预热
...
- **最终目标**：修复 deferred display 把原位 `cell-colors` patch 错发为 replacement generation 的产品错误；十二类颜色视图全部通过 GPU 常驻 attribute / palette 快切；地图装载或正式数据提交后，以最新 `mapIdentity + sourceRevision + topologyRevision + renderGeneration` 为键在共享 MapWorker 中低优先级预热仍需几何计算的共享缓存。新 revision 到来时清空未开始队列、协作取消或拒绝正在运行的旧结果，只为最新版本重算。
