...
12. 军事单位编辑面板已完成概要、比例面板、态势筛选、手动态势调整、军团重命名、批量态势、驻地/基地、战斗事件记录、事件列表筛选/清空、事件 JSON/CSV 按范围导出、事件 JSON 导入、轻量结果应用、结果预览明细和持久战报摘要：当前“军事管理”支持通过二级操作可撤销地修改单个军团态势、名称、驻地、基地，并可记录、导入、筛选、清空、按筛选清理和按范围导出战斗事件，选择性应用轻量损耗和态势变化；已应用事件会保存中文结果摘要与兵种损耗摘要。后续军事方向应继续完善更完整的多条战报链路，避免直接跳到完整战斗模拟。
