...
控制台 / 扩展 API 系统的权威任务第 28～34 项、依赖稳定 API 的第 35 项快捷键机制，以及第 36～44 项后续能力已经全部完成并移出活动清单。下一波第 45～54 项已经完成范围整理，尚未开始实现。
...
- **权威任务第 53 项：轻量纹章数据与只读图层。** `待执行；来源：emblems-coa-plan`
   - 范围：先冻结国家、省份、城市 `coa` 的兼容保存契约和详情摘要，再提供默认关闭、按需加载的简化 SVG overlay 图层及对象选择；图层进入既有 `layers` API，不实现完整生成器、编辑器、上传或 Armoria。
   - 验收：无纹章地图不增加首屏纹章资源；含旧 `coa` 数据的地图导入导出不丢字段；可解析纹章显示简化盾牌，不可解析数据显示占位且保留原文；图层分级显隐、选择、PNG overlay 开关和性能预算明确。旧数据回归、图层/选择回归、生产构建和一次浏览器图层抽查通过。
...
本次只完成计划整理和文档校准，不自动开始第 45 项。执行顺序固定为第 45～54 项；用户明确要求开始实现后，才从第 45 项进入目标模式。
