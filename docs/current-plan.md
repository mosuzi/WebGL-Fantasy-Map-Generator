...
> **执行门禁（2026-08-19）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-3` 已由同一只读评审智能体 `ACCEPT`，盘点后强制插入的 `349-3a` registry 与 identity adapter 已冻结实现并等待只读评审。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；全阶段不执行浏览器验收，只在最终阶段形成并评估浏览器验收方案。
...
- 状态：进行中，当前阶段 `349-3a` 待评审；`349-0`～`349-3` 已接受。
