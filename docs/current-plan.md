...
12. 军事单位编辑面板已完成概要、比例面板、态势筛选、手动态势调整、军团重命名、批量态势、驻地/基地、战斗事件记录、事件列表/导出和轻量结果应用：当前“军事管理”支持通过二级操作可撤销地修改单个军团态势、名称、驻地、基地，并可记录战斗事件，选择性应用轻量损耗和态势变化；后续军事方向应继续完善战报链路、导入导出回归和更明确的战斗结果说明，避免直接跳到完整战斗模拟。
