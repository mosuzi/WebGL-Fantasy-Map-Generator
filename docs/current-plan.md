...
当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、202 个公开方法和 104 个编辑方法，稳定等级为 `194 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。

继续采用目标模式：只实现第 64～81 项的封闭范围，达到当前项非浏览器门禁后立即转入下一项；不影响当前验收的新发现只写入 `FOLLOWUPS.md`。自第 76 项起按“调查 → 开发 → 代码审查”逐项实施和提交，第 76～81 项代码全部完成后再统一执行浏览器验证；第 64～75 项已经完成，不再复测。第 70 项只产出可追溯的可行性结论；第 73～78 项含显式产品规则门禁，到达对应项时先冻结清单内列出的选择，不得在未决状态下修改地图数据。
...
第 64～81 项已全部完成，当前没有其它活动权威任务。纹章深化、动态军事、贸易动画、3D / OBJ、子地图变换、真实 CRS、富文本备注和新导出格式等仍为明确暂缓或远期候选，不作为隐含待办。
