...
> **执行门禁（2026-07-30）**：第 45～52、54～216 项已完成，第 53 项已移除；上述完成项不得重新执行各自原验收或重新计入待办。当前只执行第 217 项；完成后再进入第 218 项。

当前 API 基线为：`window.webglGeneratorApi` 覆盖 `16` 个命名空间、`303` 个公开方法和 `174` 个编辑方法，稳定等级为 `295 / 7 / 1`；`303 / 303` 方法可通过 `info.describe` 发现，`planner.listRecipes / getRecipe` 只读公开 `10` 个配方和 `43` 个顶层步骤，`objects` 覆盖 `20` 类对象，`cells` 已提供八个读取、定位、扫描与动作预检方法。
...
- **权威任务第 218 项：在最终城市人口形成后确定性重评省会。** `排队；依赖第 217 项；来源：用户真实浏览器反馈并明确批准`
