...
- **权威任务第 206 项：河流干流关系可见与孤立支流生成门禁。** `已纠错完成；来源：用户在“丹水”问题后的直接补充`
  ...
  - 最小验收：固定种子生成结果中缺失父河、自指、父子不相交、父河循环、河道 cell 跳跃、伪水体出口、无出口根河及其上游支流均为 `0`；合成旧图迁移不丢河流，入湖河流不再显示为支流，无有效出口的整棵旧水系有明确异常状态；列表、详情、悬停可读出干流与出口状态；专项回归、生产构建及用户 `5410` 存档中的“丹水 / 玉溪”整条下游链复核通过。
  ...
  - 最终完成记录：归一化器现同时验证河道 cell 逐段邻接、出界边界 cell、陆水末端邻接、共享岸线上的真实河线终点、外海 Feature 边界连通和父链根出口；根河无效时旧图整棵水系保留并标诊断，新生成结果整棵拒绝。五个固定 10k 种子最终保留 `833` 条河流 / `200` 条有效支流，拒绝 `33` 条无有效出口河段，所有关系、路径和根出口断言为 `0` 缺口；合成旧图 `7 / 7` 保留。真实 `5410` 存档确认玉溪为“主河（无出口）/ 水体出口无效”，丹水为“支流（无出口）→ #48 玉溪 / 下游根河无有效出口”，悬停同步显示；当前 `258` 条旧河流未被删除或重生成。
