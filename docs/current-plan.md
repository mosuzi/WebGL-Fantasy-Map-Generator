...
> **执行门禁（2026-08-20）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-10f` 已由独立只读评审 `ACCEPT`，349-10g 已冻结待评审。该阶段只删除定义、引用、正式 owner 与专项门共同证明冗余的 `revisionProfile`；连续两次专项首败确认 markers 能力夹具仍停留在 349-7、与 349-10d 已接入的正式 Worker / regeneration 契约漂移，因此动态插入 `349-10g-a`，未完成顺序为 `349-10g -> 349-10g-a -> 349-11`。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，最终阶段只形成并评估浏览器验收方案与防误触门。
...
- 状态：进行中，`349-0`～`349-10f / 0.5.47` 已接受，`349-10g` 已冻结待评审；动态插入 `349-10g-a` 后再进入 `349-11`。
...
- 阶段：`349-0` 计划冻结；`349-1` 现状盘点；`349-2` TS 工具链；`349-3` 核心契约；`349-3a` 字段注册表与身份适配器闭合；`349-4` Manifest；`349-5` 薄 facade；`349-6` notes 切片；`349-7` markers 切片；`349-8` Worker 切片；`349-9` dependency / projection；`349-10a / 10b` 已接受；动态插入 `349-10c0` 共享 Worker result ownership；随后 `349-10c`～`349-10g` 依赖序逐域迁移与旧路径收口；动态插入 `349-10g-a` 同步 markers 能力夹具；`349-11` 非浏览器终验与浏览器验收方案。
