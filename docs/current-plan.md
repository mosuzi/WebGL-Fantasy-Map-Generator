...
- **权威任务第 335 项：GPU 常驻视图与零重编译切换。** `进行中；来源：用户要求根治 100k 视图切换端到端卡顿，不能再用 Worker 算法耗时冒充上屏性能`
  ...
  - **阶段状态**：335-A～335-G 已接受并分别以 `0.3.17 / 0.3.18 / 0.3.19 / 0.3.20 / 0.3.21 / 0.3.22 / 0.3.23` 建立本地 checkpoint。固定 100k 暖普通颜色为 `101～186ms / 3` 输入包；revision 推进后的 cache 失效为 `5.02s`，session 丢失 cold 为 `21.07s / 1032` 包。B～C 建立稳定 packed surface 与 GPU cell attribute / palette；D～F 让普通 / 政治颜色、主题、海底和普通图层本地提交；G 把平滑内部边界收敛为硬基座上的 packed cell correction，并由 Worker 预备平滑岸线面与平滑 / 硬岸线线条缓存。固定 `99846` cells 首次开启、关闭、重复开启为 `20.2 / 18.7 / 16.4ms`，与旧 Worker framebuffer 像素差异为 `0`，Worker、完整 surface / line 重建和 LongTask 均为 `0`。当前唯一施工阶段为 335-H。
