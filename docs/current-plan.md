...
12. 军事单位编辑面板已完成概要、比例面板、工具条分组、态势筛选、手动态势调整、军团重命名、批量态势、驻地/基地、只读战役对象、战斗事件记录、事件列表筛选/清空、事件 JSON/CSV 按范围导出、事件 JSON 导入、轻量结果应用、结果预览明细、持久战报摘要、战报链摘要、链路分组筛选、可点击链路概览和攻防双方损耗摘要：当前“军事管理”支持通过二级操作可撤销地修改单个军团态势、名称、驻地、基地，并可选择链路记录、导入、筛选、清空、按筛选清理和按范围导出战斗事件，选择性应用轻量损耗和态势变化；军事生成会提供 `map.military.campaigns` 战役摘要，面板统计、参战军团详情和军事 JSON 导出都会带出战役数、战役名、双方兵力、关联 front 与共享 `campaign:*` 链路；已应用事件会保存中文结果摘要与兵种损耗摘要，选中军团详情、战斗记录面板标题、战报链摘要和 CSV/JSON 事件导出都会带出共享战役链路、进攻/防守方、对手国家、事件序号、应用状态、损耗状态、攻方/守方损耗分桶和筛选上下文，选中军团事件超过 5 条时可展开完整筛选链路，并可按链路、类型、结果、已应用/未应用筛选。后续军事方向应继续做真正双方结算、战役推进和战争行动链路，避免直接跳到完整战斗模拟。
