...
当前已提交并推送的 API 基线是 `257 / 135`；第 208 项步骤 5 完成后的本地工作区基线已扩展为：`window.webglGeneratorApi` 覆盖 `15` 个命名空间、`283` 个公开方法和 `156` 个编辑方法，稳定等级为 `275 / 7 / 1`；`283 / 283` 方法可通过 `info.describe` 发现，`objects` 覆盖 `20` 类对象，`cells` 已提供八个读取、定位、扫描与动作预检方法。本批全部统一验收后再推送该工作区基线。
...
- **权威任务第 208 项：把五个多 API 碎片收敛为原子规则事务。** `执行中；来源：用户整体批准第 204 项缺口`
