...
2026-07-18 第 64～81 项全部完成并统一验收、推送。用户随后明确批准的五项界面与生成体验整改已按原始顺序登记为第 82～86 项并全部完成；本轮没有从历史缺口或候选文档追加其它任务。用户又反馈悬停菜单会被国家、省份名称遮挡，现按连续编号登记为第 87 项。

第 87 项继续采用“调查 → 开发 → 代码审查”流程并单独提交，代码完成后再执行本项浏览器验证；第 64～86 项不得复测。不影响本项验收的新发现只记录到 `FOLLOWUPS.md`。当前活动权威任务只有第 87 项。
...
> **执行门禁（2026-07-18）**：第 45～52、54～86 项已完成，第 53 项已移除；上述完成项不得复测或重新计入待办。当前只执行第 87 项，不得追加其它整改。
...
第 87 项按“调查 → 开发 → 代码审查”实施并本地提交，代码完成后只统一验证本项；第 64～86 项不得复测。验收通过后再统一推送包含本项计划、实现与验证的全部提交，中途不推送。
...
- **权威任务第 87 项：修正悬停菜单被国家、省份标签遮挡。** `待执行；来源：用户 2026-07-18 反馈`
  - 痛点：地图悬停菜单当前所处堆叠层级低于国家和省份名称标签；两者覆盖区域相交时，标签会绘制在菜单上方，遮挡菜单内容并干扰操作。
  - 范围：先调查悬停菜单、统一地图标签 overlay 及相关 stacking context 的真实 DOM / CSS 层级，再只调整悬停菜单与国家、省份标签之间的前后关系，使菜单打开期间稳定位于政治标签上方。保留标签内部的国家 / 省份 / 城市优先级、碰撞与点击语义，不全局抬高所有面板、下拉框、对话框或 tooltip，也不改变悬停菜单的触发、定位、内容和关闭行为。
  - 验收：在国家名和省份名分别与悬停菜单发生重叠的代表性场景中，菜单全部文字、背景和动作入口均完整显示在标签上方并可点击；菜单关闭后标签显示与交互保持原样，缩放、平移和窄视口下不出现新的遮挡或穿透。补充层级 / stacking context 专项回归，相关标签与悬停菜单回归、生产构建和 `git diff --check` 通过；代码完成后只对第 87 项执行浏览器重叠场景验证。

第 64～86 项已全部完成；当前活动权威任务只有第 87 项。纹章深化、动态军事、贸易动画、3D / OBJ、子地图变换、真实 CRS、富文本备注和新导出格式等仍为明确暂缓或远期候选，不作为隐含待办。
