...
当前唯一仍未全面完成的高优大项是控制台 / 扩展 API 系统。它已经完成阶段 0～6 第一刀，但尚未达到“全量能力覆盖、UI / API 共路径、统一异步事务、旧数据完整证明、稳定版本契约和一键聚合门禁”的整体目标，因此拆为权威任务第 28～34 项继续实施。
...
下一波只继续尚未全面完成的“控制台 / 扩展 API 系统”。当前基线是：`window.webglGeneratorApi` 已覆盖 10 个命名空间、127 个公开方法和 46 个编辑方法，阶段 0～6 均已有第一刀，方法元数据覆盖自检为完整，并已有 capabilities、完整地图 roundtrip、GEO、备注 / 测量导出、名称库和名称库改名回归。尚未完成之处是：缺少全量能力盘点与封闭范围，部分既有 edit command 尚未暴露，UI / API 仍未全部收束到同一 action 层，异步长任务状态未统一，所有接口仍是 `experimental / draft`，也没有一条覆盖整套 API 的聚合门禁。
...
执行顺序固定为 `28 -> 29 -> 30/31 -> 32 -> 33 -> 34`。每项达到本节最小验收后立即进入下一项；新发现但不影响本项验收的增强只记入 `FOLLOWUPS.md`，不扩展完成标准。
