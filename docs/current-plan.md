...
- **2026-07-30 用户直接修正：控制面板固定 Tab 与开发模式按钮单行。** `已完成；来源：用户直接要求`
  - 范围：控制面板顶部七个 Tab 固定在面板正文上方，不再随长内容一起滚动；只有当前 Tab 的正文区域承担纵向滚动。“开发模式”工具栏按钮文字保持单行，窄屏时允许整个按钮随工具栏换到下一行，但不允许按钮内部折行。
  - 最小验收：管理 Tab 等长内容滚到底时，父 `.floating-panel-body` 不滚动、Tab header 位置变化不超过 `1px`、正文可到达末项；普通与 150% 字体下“开发模式”按钮均为单行且没有文字或 document 横向溢出。七个 Tab 点击 / 键盘切换、开发面板入口、application console、page 与 WebGL error 均保持正常。
  - 排除：不改变共享 `PanelManager`、其它浮动面板滚动语义、普通工具按钮的安全换行、Tab 顺序或控制面板父子互斥行为。
  - 完成记录：只为 `generation-panel` 建立外层固定、根容器纵向 flex 和正文独立滚动链；开发入口仅覆盖 `max-content / nowrap`，继续允许整个按钮随工具栏换行。响应式专项同步冻结两项契约，并按第 218 项新增的省会重评二级动作把复杂工作区分母从 `18 / 68 / 55` 校准为 `18 / 69 / 56`。生产构建、控制信息架构、UI system 契约与差异检查通过；系统 Chrome 在 `1280×720 / 720×720 / 576×576` 三档均测得父区 `scrollTop=0`、Tab 位移 `0px`、正文可滚到底且 document 横纵溢出为 `0`，七个 Tab、方向键和开发入口正常。576px 字体压力下按钮整体换到下一行，文字仍为单行；application console、page 与 WebGL error 均为 `0`，两路独立复核均为 `ACCEPT`。
