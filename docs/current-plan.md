...
- 生物群系 / 气候 / 水体地貌 / 人口统计面板列宽拖拽持久化第一刀已接入：四个统计面板 defaults 现在声明 `columnWidths`，并复用 `UiObjectTable` 的 `resizable-columns` / `column-resize` 读写闭环；拖动列宽会写回各自 `panel-list-preferences.columnWidths` 并刷新 state。本步只扩展四个只读统计表格，不改变生物群系、气候、水体地貌或人口统计数据、排序、筛选、选中详情或专题派生逻辑。验证证据待本批次统一烟测后回填。
