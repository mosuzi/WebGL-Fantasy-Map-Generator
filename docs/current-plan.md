...
控制台 / 扩展 API 系统的权威任务第 28～44 项、曲线尺第 45 项、备注治理第 46 项、名称库收尾第 47 项、市场归属第 48 项、路线编辑第 49 项、水体与 feature 局部修正第 50 项、生物群系局部编辑第 51 项和界面审计整改第 55～60 项已经全部完成并移出活动清单。现在从第 52 项继续执行第 52、54 项；原第 53 项已按用户决定移除，短期不深化纹章系统。
...
> **执行门禁（2026-07-15）**：用户已批准按目标模式实施。第 45～51、55～60 项均已达到最小验收并移出活动清单；现在执行第 52 项，后续完成第 54 项。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、180 个公开方法和 90 个编辑方法，稳定等级为 `172 / 7 / 1`；第 28～51 项均已达到各自封闭范围内的最小验收并移出活动清单。

下一波继续采用目标模式：只实现当前编号项的封闭范围，达到最小验收后立即转入下一项；不影响当前验收的新发现只写入 `FOLLOWUPS.md`。日常实现以代码检查和专项回归为主，只有验收明确要求时才运行浏览器。

- **权威任务第 52 项：用户主题导入导出与颜色级编辑。** `执行中；来源：visual-theme-preset-plan`
...
第 45～51、55～60 项已完成并移出活动清单；当前执行第 52 项，第 54 项随后执行。第 53 项已移除且不作为隐含待办。
