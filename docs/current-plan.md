...
12. 军事单位编辑面板已完成概要、比例面板、态势筛选、手动态势调整、军团重命名、批量态势、驻地/基地、战斗事件记录、事件列表/JSON/CSV 导出、轻量结果应用、结果预览明细和持久战报摘要：当前“军事管理”支持通过二级操作可撤销地修改单个军团态势、名称、驻地、基地，并可记录战斗事件，选择性应用轻量损耗和态势变化；已应用事件会保存中文结果摘要与兵种损耗摘要。后续军事方向应继续完善导入兼容回归和更完整的战报链路，避免直接跳到完整战斗模拟。
