...
- **权威任务第 216 项：统一焦点、长文本响应式与管理编辑弹框高度。** `活动；来源：Q-32，用户明确批准并补充高度要求`
  - 范围：建立共享 `focus-visible` token 并覆盖共享按钮、关闭、排序、列宽、空态、checkbox 和常驻工具入口；长中文在窄视口和 150% 字体下允许安全换行、省略或收纳，不得依赖 document 横向滚动。
  - 高度规则：管理 Tab 打开的主编辑面板和动作坞二级编辑弹框必须以最终实际 `top`、真实锚点矩形和视口安全区计算可用高度。主面板上限为 `hostHeight - safeTop - bottomMargin`；二级弹框分别计算锚点行上下空间，优先选择可容纳侧，否则选择较大侧，并以该侧真实空间作为 `max-height`。控制面板入口、Tab 行和管理入口行必须保持可见；header 固定、body 内部滚动，禁止按当前字体写固定魔法高度。
