...
- 本地左键拖拽 overlay 漏洞已补齐：左键单击仍用于选择对象，但左键按下后移动超过阈值会切换为平移并进入 `drawViewportPreview()`，因此普通左键拖动画布时也会临时隐藏 `map-overlay`、标签、城市剪影和军事图标。加载完成前的滚轮 / hover 也加了 map 未就绪保护，避免 overlay 被异常卡住。Vite dev 浏览器烟测覆盖加载前滚轮、加载后滚轮、中键拖拽、左键拖拽、适配视图和对象定位，均无 console/page error；左键拖拽期间 city/state label、city icon、military icon 的父链有效可见性均为 `false`，idle 后恢复。100k `viewport-overlay-left-drag-100k` 完整图层滚轮 / 中键拖拽 frame p95 为 `6ms / 17.6ms`，overlay 暂停样本 `18 / 18` 和 `24 / 24`，idle dirty 恢复 `clean`；10k e2e WebGL 加载 `372.1ms`。
...
2. **overlay 与动态线层性能专项**：
   - `profile:overlay` 已补 idle commit 指标和 `measurement-heavy / selection-heavy` 重场景；viewport idle commit 分帧第一刀、程序化视口变换统一隐藏补洞均已完成。
   - 后续如果继续性能专项，先复查 100k 下偶发 long task 和 overlay 恢复更新 `25-43ms` 的来源；只有证据继续指向河流、选中态或 overlay 更新时，再做河流异步 mesh、选中 mesh 分帧、过期 commit 取消增强、视口分块或缓存。
