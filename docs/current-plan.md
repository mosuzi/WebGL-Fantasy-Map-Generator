...
> **执行门禁（2026-07-16）**：第 45～52、54～63 项已完成；第 53 项已移除。当前只执行第 64～81 项，顺序固定为 `64 -> 65 -> 66 -> 67 -> 68 -> 69 -> 70 -> 71 -> 72 -> 73 -> 74 -> 75 -> 76 -> 77 -> 78 -> 79 -> 80 -> 81`，前一项未验收前不得开始后一项。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、190 个公开方法和 92 个编辑方法，稳定等级为 `182 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。
...
- **权威任务第 74 项：国家合并、拆分与政治拓扑编辑。** `Queued；原准权威 Q-04；执行首步冻结继承规则`
...
- **权威任务第 75 项：完整海岸雕刻与 Feature 类型转换。** `Queued；原准权威 Q-05；执行首步冻结允许转换矩阵`
  - 痛点：当前只有受限湖岸扩展 / 收回，尚不能系统修补海岸、开闭海峡或安全转换岛屿、湖泊、海洋等 feature 类型。
  - 产品门禁：实施前列出并批准允许的类型转换组合、最小海峡宽度、开放海域判定和禁止操作；不得用一个无约束画笔任意翻转水陆。
  - 范围与验收：提供只读预检、受约束画布预览和单事务应用；同步 grid / pack 高度与 feature、岸线、haven / harbor、水文、邻接、picking、标签、统计和导出，受影响派生系统按契约重建或标记 stale。非法拓扑有明确原因且不改图；撤销 / 重做、完整地图往返、专项回归、生产构建、`git diff --check` 和代表性开闭海峡浏览器验收通过。

- **权威任务第 76 项：文化与宗教中心、扩张参数编辑。** `Queued；原准权威 Q-06；执行首步冻结联动规则`
...
- **权威任务第 77 项：数值适居度笔刷。** `Queued；原准权威 Q-07；执行首步冻结数值语义`
...
- **权威任务第 78 项：人口手工调整第一刀。** `Queued；原准权威 Q-08；执行首步冻结总量语义`
...
- **权威任务第 79 项：补齐 Source 地形模板操作。** `Ready；原准权威 Q-09`
...
- **权威任务第 80 项：标签显示优先级与位置锁定。** `Ready；原准权威 Q-10；依赖第 69 项标签类型 registry`
...
- **权威任务第 81 项：补齐路线与河流 GeoJSON 稳定属性。** `Ready；原准权威 Q-11`
...
除第 64～81 项外，当前没有其它活动权威任务。纹章深化、动态军事、贸易动画、3D / OBJ、子地图变换、真实 CRS、富文本备注和新导出格式等仍为明确暂缓或远期候选，不作为隐含待办。
