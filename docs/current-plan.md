...
2026-07-16 用户补充三个国家生命周期问题，冻结为第 61～63 项：首都省份创建保护、新建国家疆域着色扩张、删除国家档案彻底移除。三项按编号逐项达到最小验收并独立提交，全部完成后统一执行集中验证与推送。
...
> **执行门禁（2026-07-16）**：第 45～52、54～63 项已完成；当前没有活动权威任务，正在执行本批次集中验证与统一推送。
...
- **权威任务第 61 项：禁止在既有国家首都所在省份创建国家。** `已完成；来源：用户实测`
  ...
  - 完成记录：新增共享 `inspectStateCreation()`；画布新增、API 新增和命令 `apply` 共用首都省份保护。固定地图的首都 cell `578` 与同省 cell `644` 均返回 `capital-province-protected`，完整政治 / 城市 / cell 快照与历史保持不变；允许 cell `547` 创建国家 #18，并完成撤销 / 重做。
...
- **权威任务第 62 项：新建国家可立即着色扩展疆域。** `已完成；来源：用户实测`
  ...
  - 完成记录：国家笔刷的正常 `pointerup` 已接回 `finishStateStroke()`，`pointercancel` 改为只回滚预览。固定地图新建国家 #18 后扩张 grid cell `682` 与 pack cells `96 / 97`，国家统计增加且只写入一条历史；撤销 / 重做恢复一致。静态门禁确认新国家仍同时进入面板目标与地图 selection。
...
- **权威任务第 63 项：删除国家后彻底移除国家档案。** `已完成；来源：用户实测`
  ...
  - 完成记录：国家删除不再保留 `removed` 档案，而是在不重排 ID 的前提下把 `politics.states[id]` 与独立 `pack.states[id]` 置空；国家面板桥接层、Vue 列表与对象解析器兼容过滤旧 tombstone。国家 #18 删除后 grid / pack / 城市无归属残留、原省份移除、对象解析及完整地图 / GeoJSON 导出均无该国；撤销完整恢复，重做再次移除。

当前权威任务清单为空。第 45～52、54～63 项已完成并移出活动清单；第 53 项已移除且不作为隐含待办。本批次只剩集中验证与统一推送。
