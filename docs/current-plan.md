...
- marker / 资源点管理面板第一刀已完成：管理 tab 新增“资源标记”入口，浮动面板支持全部/资源点/普通标记切换、筛选、排序、选中、定位、详情、重命名、近景图标 `symbol/palette` 手动调整和撤销/重做；暂不做新增、删除、移动和局部重算。
...
2. marker 后续不要为了单个类型继续扩大总量；下一步可进入 marker 新增/删除/移动、局部重算，或把城镇剪影继续扩展成更细的文化图标包，而不是再调全局 target。可追加的资源点类型包括：采石场、黏土坑、煤田、硫磺泉、硝石洞、琥珀海岸、珍珠滩、珊瑚礁、渔场、良港、森林木场、树脂林、药草谷、染料草场、香料林、茶山、丝茧桑园、马场、牧盐草甸、绿洲、圣泉、地热田。
