...
2026-07-23 用户明确批准在并行分支使用共享边界拓扑实验室的最佳算法完成一版初级海岸平滑；合并回 main 后，为避开 main 已使用的第 204～206 项，顺延登记为权威任务第 207 项。本项只进入正式 renderer 的海岸视觉阶段，不顺延国界、省界、存档拓扑或编辑事务。

2026-07-23 用户实测指出第 207 项开启与关闭“平滑边界”没有可感知区别。真实 Chrome 同相机对照与逐弧统计确认开关已切换，但主要大陆海岸因整弧安全回退保持原样，已接受弧线的位移又不足一个屏幕像素；按连续编号登记为权威任务第 208 项，只修正海岸平滑的用户可见强度与局部安全回退。

2026-07-24 用户提供真实地图截图，确认凹岸与大转折处出现旧政治填色穿出平滑海岸线的三角尖刺；按连续编号登记为权威任务第 209 项，先在共享边界拓扑实验室加入“填色—描边分离”夹具与硬门禁，再修正正式 renderer 的同源填色覆盖。

2026-07-24 用户复核后再次提供真实地图截图，确认第 209 项完成后仍有大面积蓝色 / 浅色三角扇穿过海岸。最终 GPU 三角面重放定位到自适应海岸过渡带只检查两条外缘，未检查实际提交的四个三角形；法向急转时即使旧门禁标记为安全，仍会生成反向或交叉三角面。按连续编号登记为权威任务第 210 项，在实验室固定该反例，并让 renderer 与回归直接约束最终三角面。

2026-07-24 用户继续复核同一区域，指出大三角扇消失后，原始岸线与最终共享岸线的收敛端仍会留下单像素级陆色 / 水色针尖。截图取色确认左侧针尖为精确陆地填色 `rgb(189,231,239)`，不是透明缝或描边；正式 10k 修补面统计也包含大量极窄三角形。按连续编号登记为权威任务第 211 项，在不退回旧四三角过渡带的前提下收敛 XOR 差区的亚像素尖端，并保持面线同源与对象保护。

2026-07-24 用户再次复核确认第 211 项仍未解决，并要求实验室原样复刻截图中的两处缺陷。此前只截取局部白色像素链、以抽象 `0.171px` V 形夹具代替真实水道与岸线汇合的验收无效；第 211 项完成状态撤回，重新进入“正式地图精确取证 → 实验室同构复现 → renderer 修正 → 同区域复核”。

2026-07-24 用户第三次复核确认“低像素 XOR 修正面淘汰”后缺陷仍在。第二次完成结论再次撤回：实验室虽使用相同配色和近似构图，但仍是手工绘制的示意面，没有从正式 renderer 提取实际 GPU 三角形与 draw batch；正式验收也因页面重载丢失原运行时地图和相机，未在用户指出的同一图面闭环。第 211 项继续执行，下一步必须先用正式运行态的 draw-call / 顶点归属证据判定缺陷来源，再修改对应 writer。

2026-07-24 正式 `stage-2-1 / 10k` surface 顶点取证确认缺陷不属于 XOR 修补批次，而是基础 `cell-visual-layer`：曲线化后的凹 cell 仍以原 cell center 做三角扇，部分扇面越过自身多边形。第 211 项据此更正为基础填色三角化修复；实验室必须直接使用正式 cell 顶点，不能继续使用截图近似。

2026-07-25 用户再次确认边界填色瑕疵仍未解决。第三次完成结论撤回：`#1061 / #8832` 证明了正式基础 cell 中存在中心扇越界，但没有证明它们就是用户当前页面残留像素的全部来源；“同类真实反例”仍不能替代用户当前浏览器、当前地图、当前相机的逐像素批次归属。第 211 项重新执行，必须保留现场并以当前像素为唯一验收坐标。

2026-07-25 用户第五次复核仍在同一区域看到两处残余：左下蓝色水面内的长白针，以及右侧水陆斜边上的浅色分离带。第四次完成结论撤回；此前恢复 `v5331 / v5519` 的 `0.99 CSS px` 共享边只修复了一个坍缩点，没有覆盖本次截图中的全部可见缺陷。第 211 项继续执行，并要求把这两处正式页面像素原样复刻到共享边界拓扑实验室。
...
2026-07-25 用户要求根据第 207～211 项连续返工重新审视共享边界拓扑实验室，优化既有用例并设计更复杂、容易触发真实缺陷的复合用例，同时尝试修复新门禁暴露的问题；按连续编号登记为权威任务第 212 项。本项只加强海岸 / 湖岸的生产同源取证、光栅像素门禁、局部回退拼接和安全三角化，不扩展到国界、省界或新的通用编辑能力。该专项在并行分支先行执行；合并时第 200 项及第 195 项 A～D 已由 main 完成，不再沿用当时的待执行描述。
...
> **执行门禁（2026-07-28）**：第 45～52、54～211 项已完成，第 53 项已移除；上述完成项不得重新执行各自原验收或重新计入待办。`Q-25～Q-27` 已转为第 201～203 项并完成，只有 `Q-28～Q-33` 仍未批准。第 205 项重生成锁定体系已完成，第 212 项海岸高风险实验矩阵是当前唯一活动专题；不得从候选、历史记录或矩阵缺口自行创建后续实施项。

当前已提交并推送的 API 基线是：`window.webglGeneratorApi` 已覆盖 `15` 个命名空间、`257` 个公开方法和 `135` 个编辑方法，稳定等级为 `249 / 7 / 1`；`257 / 257` 方法可通过 `info.describe` 发现，`objects` 覆盖 `20` 类对象，`cells` 已提供八个读取、定位、扫描与动作预检方法。
...
- **权威任务第 207 项：推荐共享管线的初级海岸平滑。** `已完成；来源：用户直接批准；并行分支旧编号 204`
...
- **权威任务第 208 项：让海岸平滑在正常视距下可感知。** `已完成；来源：用户直接反馈；并行分支旧编号 205`
  - 问题证据：第 207 项正式样本的 `1756` 个海岸源点中，实际进入平滑弧线的源点只有约 `121` 个，约为 `6.9%`；`628 / 301 / 102 / 573` 点等主要大陆弧线因单点陆水侧或沿岸对象门禁失败而整弧回退。当前 `0.04` Chaikin 的已接受弧线最大位移不足一个屏幕像素，开关虽切换 renderer 路径，但不满足用户可见目标。
...
- **权威任务第 209 项：在实验室复现并消除海岸填色—描边分离。** `已完成；来源：用户截图反馈；并行分支旧编号 206`
  - 问题证据：用户截图中的凹岸、狭长半岛和急转弯处可见土地填色三角尖刺越过灰色平滑海岸线，局部还出现海水楔形切入陆地。第 208 项回归只检查过渡带自身的非零宽覆盖、中心 / 外缘 cell 颜色、接缝和几何安全，没有测量原始 cell 填色相对最终平滑中心线的残留外露，因此真实反例未被拦截。
...
- **权威任务第 210 项：消除自适应海岸过渡带的翻面三角扇。** `已完成；来源：用户复核截图；并行分支旧编号 207`
  ...
  - 最小验收：固定反例在旧门禁下可复现、修复后不再输出反向三角形；正式 10k 海岸 / 湖岸、高度 / 国家 / 省份三模式的最终过渡带三角形反向数为 `0`，不绘制 `geometryFallback`；第 209 项 XOR 独立重放仍为错侧、重复覆盖、陆水冲突 `0`；沿岸城镇、道路、河流、河口与开关语义不变，生产构建、真实 Chrome 复核和 `git diff --check` 通过。
  - 排除：不改变共享海岸算法、平滑强度、canonical 地图、国界 / 省界、编辑或存档；XOR 已保证最终填色与描边同源，无法安全三角化的局部渐变只允许收尖或省略，不得以继续输出反向面来维持装饰带连续。
  - 完成记录：最终 writer 四三角重放确认真实固定段的有向面积为 `-6.074 / 4.631 / 4.631 / 7.623`，旧“外缘不相交”门禁漏掉一个反向面；对五个确定性 10k 种子抽查时，大量旧输出也存在同类翻面，因此没有采用会造成渐变忽明忽暗的逐段补丁。第 209 项 XOR 已精确把原始填色修正到最终共享中心线，正式 renderer 现只提交该修补面与同源描边，不再叠加冗余四三角过渡带；拓扑统计明确报告 `visualBand=retired-after-exact-xor`。实验室新增第十个“海岸过渡带翻面”夹具，左图标红反向三角扇，右图固定显示最终过渡带三角面 `0`，推荐矩阵 `10/10`。正式 10k 仍输出 `1890` 个 XOR 修补三角形，独立重放 coastline `5175`、lake-shore `195` 个差异采样，错配、重复覆盖、陆水冲突均为 `0`；高度 / 国家 / 省份平滑与硬边六状态系统 Chrome 回归通过，平滑切换 `174 / 218.1 / 209.6ms`，`glError = 0`、console / page error 为 `0`。真实近景凹岸和狭窄水道未再出现蓝色 / 浅色三角扇，沿岸对象关系未变；生产构建 `1253` modules 与差异检查通过。

- **权威任务第 211 项：消除 surface 的像素级异侧填色针尖。** `已完成；来源：用户第五次复核；并行分支旧编号 208`
  ...
  - 排除：不恢复第 210 项已停用的自适应四三角过渡带，不扩大平滑强度，不修改 canonical 地图、国界 / 省界、编辑、存档或导入导出。
...
- **权威任务第 212 项：以生产同源和像素重放重构海岸高风险实验矩阵。** `执行中；来源：用户直接要求；并行分支旧编号 209`
  - 调查结论：现有最后五个 surface 夹具并非全部消费正式 writer；翻面夹具直接声明最终三角为空，顶点坍缩夹具直接保存预期精确边，第十三项又通过 `finalBoundaryCoverWorld > 0` 推导“无裸露边缘”。世界坐标格点采样没有覆盖 Float32 上传、WebGL top-left 边缘所有权、DPR、相机非等比缩放、子像素相位、绘制顺序和批次归属，因此 `13/13` 仍可能对正式 GPU 像素给出假阳性。
  - 封闭用例：新增四类固定高风险夹具——正式 draw packet 的 Float32 光栅相位矩阵；外海岸、湖洞、窄水道和凹角共存的多环 XOR；平滑段 / raw fallback 段与城镇、道路、河流、河口相邻的局部拼接；重复点、极短边、强凹 cell 的 Earcut 安全失败路径。每类必须显示可定位失败区域，并至少有一个删除覆盖、改错方向、错序、量化端点或强制 Earcut 空输出的破坏反例。
  - 修正边界：实验室与回归必须直接消费生产纯函数或生产 writer 的结构化输出，不得再用“目标常量已写入”替代结果重放。只有新增红灯证明正式实现存在缺陷时，才按“安全三角化 fallback → 边缘覆盖方向 / 去重 → 投影尺度 → 缓存失效”的顺序做最小修正；不预先扩写算法。
  - 最小验收：原 13 个与新增 4 个夹具全部通过，四类破坏反例全部稳定失败；像素夹具至少覆盖 DPR `1 / 1.5 / 2`、三档缩放、横纵非等比投影和多个子像素偏移，最终异侧纯色像素、最长连续针、陆水冲突和内部重复覆盖均为 `0`。凹 cell 禁止回退会越界的中心扇；局部回退拼接保持端点、canonical cell 与四类保护对象。实验室专项、正式 shoreline / cell surface 专项、生产构建、差异检查和真实浏览器实验室 / 正式地图复核通过。
  - 当前阻断：实验室已扩为 `17` 个用例，Float32 top-left 光栅、DPR / 缩放 / 子像素相位、多环 XOR、局部 fallback 拼接、对象保护和安全 Earcut 均已纳入；但正式 `shore-review / 50k` 的困难封边仍有 `20` 个 render junction 像素未闭合。write-once side-depth 模型已把新增同侧 owner 污染降为 `0`，剩余像素分布于 `6` 个 path，其中 `15` 个发生在相邻 canonical owner 切换处，另外 `5` 个属于补片半径或 render 边 top-left 归属。严格同 owner 的单楔 / 小圆盘补片没有通过门禁，逐困难边重建完整 owner 布尔区又使 packet 冷构建达到 `21466.97ms`，超过 `5000ms` 门槛，因此不得继续靠放宽覆盖或忽略 owner 放行。
  - 待用户决策：后续只能在“局部 owner 分区 junction 补片（较小，但需继续验证 owner 切换和双侧像素门禁）”与“沿岸 cell surface 一体化重三角化（范围更大，但从几何源头统一填色 ownership）”之间选择。当前不得标记完成、提交或推送；已有半成品 side-depth 接线也不能作为正式通过结果。
  - 新增单 cell 反例：用户截图中的上方孤岛是独立闭合海岸，附近大陆几何不会参与该 arc 的 `Visvalingam + Chaikin` 处理；真正缺陷是闭环错误复用开放弧线的端点锁定，把任意 `source[0]` 强行钉回处理后环首尾，且尖角校验从索引 `1` 开始，恰好漏掉接缝索引 `0`。正式管线现只锁开放弧两端，普通闭环闭合在处理后的首点，并让闭环尖角校验覆盖全部顶点。
  - 实验室新增第十八项“单 cell 闭环接缝毛刺”：同画布固定上方单 cell 岛与下方近邻大陆，推荐结果必须释放任意原始首点；把大陆移除后的孤岛坐标必须与组合场景逐点一致。带河口或 `syntheticAnchor` 的闭环继续按语义保留锚点，不能用本次修复破坏对象保护。实验室推荐矩阵 `14/14`、总夹具 `18`，生产构建通过；正式 shoreline `--pure` 已执行到本次新增断言并通过，随后仍被第 212 项现有 surface writer 契约 / render junction 未闭合问题阻断，因此本项继续保持执行中，不据此提交或推送。
  - 新增闭环首点漏检描边反例：同一正式页面逐层隔离确认，平滑开启时横穿单 cell 岛的细针会随“水陆线”关闭而消失，岛屿填色保持不变；缺陷因此属于岸线 stroke，不是 cell surface、XOR 修补面、道路或近邻大陆。根因是位移裁剪后的 `filterShoreRenderSpikes` 仍按开放折线检查，只遍历内部点，闭环数组的首尾接缝索引 `0` 从未参与去刺。实验室现提供浏览器可直接加载的纯函数模块，正式 `shore-layer` 复用同一实现：闭环先移除重复尾点，两轮环形检查所有唯一顶点，再以新的首点恢复闭合；开放弧线语义不变。
  - 实验室总夹具扩为 `19`，新增“岸线闭环首点漏检跨面描边针”同画布复刻附近大陆、单 cell 岛、城镇和道路；左侧破坏反例固定显示残针 `2`，右侧生产同源结果为残针 `0` 且保持闭合。专项 `19/19`、推荐拓扑夹具 `14/14`、生产构建 `1259` modules 和正式地图 / 实验室 Chrome 复核通过；正式地图在 `20 千米` 原区域执行平滑关 / 开往返后未再出现横穿细针，页面应用错误为 `0`。正式 shoreline `--pure` 的新增描边断言已通过，随后仍停在既有“关闭平滑时不得保留海岸过渡面”静态契约，因此第 212 项整体继续保持执行中，不据此标记完成、提交或推送。
  - 新增视觉三角岛反例：当前 `stage-2-1 / 10k` 页面中两座岛都不是受附近大陆影响的跨 path 结果，而是各自独立闭环；视觉上的三角形实际各含一条约一世界单位的微边。旧 `simplifyClosed` 对四唯一点直接返回，使有效三角形没有进入简化；随后孤岛被旧 `land-water-side` 侧向带预检整环回退，带城镇 / 道路的岛又因 `protected-town` 回退，最终保留硬角。
  - 正式管线现允许四唯一点闭环简化为有效三角形，三角形跳过已淘汰的旧侧向带预检，由精确 XOR 负责最终陆水分类；Chaikin 系数在位移门禁内自适应二分。若闭环含保护对象，则保留保护点附近的原始微边楔，只平滑其余开放弧并重新闭合，仍需通过位移、陆水和对象门禁。实验室新增第二十项“`三顶点孤岛平滑整环回退`”，同时固定带对象岛、孤立岛和不参与计算的近邻大陆，左侧复刻 `protected-town / land-water-side` 整环回退，右侧直接消费正式纯函数并显示两岛圆角结果。
  - 专项实验室 `20/20`、推荐拓扑夹具 `14/14` 与生产构建 `1259` modules 通过。Chrome 在 `http://127.0.0.1:5432/` 保存并重载同一地图后确认：孤立岛整体圆滑，带对象岛除城镇 / 道路锚点保留小岬角外其余边界已圆滑；`http://127.0.0.1:5401/` 同名实验室用例显示正式同源最大位移 `3.75 / 18`、最终错侧像素 / 最长针 / 冲突覆盖均为 `0`，全量 `20/20`。正式 shoreline `--pure` 的新增三角断言已通过，随后仍停在既有关闭平滑静态契约；`--pure-formal` 当时仍保留 50k 三组 owner 像素失败，后续实际底面 / XOR 基线统一已使正式 10k / 50k 重放全部通过。第 212 项整体继续执行中，不据本次局部修复标记完成、提交或推送。
  - 新增实际底面 / XOR 基线漂移反例：对用户保留的 `stage-2-1 / 10k`、校验和 `35df777b` 地图逐批取证后，当前两处毛边分别落在水陆 cell 对 `#8252/#8374` 与 `#7660/#7783`。XOR 修补面使用原始 Voronoi 直岸，但基础 `cell-visual-layer` 又把同一水陆共享边按两侧 cell 独立弯曲；两个合法批次因基线不同，在当前投影下分别形成 `0.55 / 1.24 CSS px` 的近共线长针。正式 visual cell mesh 现先完整识别水陆共享边，并让这些边保持与 XOR 同源的解析后直线；同侧 cell 边仍保留曲线化，不把整张底面退回硬多边形。
  - 实验室既有像素残余夹具更名并重构为“实际底面与 XOR 补面基线漂移”，直接固定上述两组正式 cell、原始边、旧底面曲线与修补三角；左侧显示两组漂移，右侧显示“水陆边直岸同源”且残针 `0`。专项实验室 `20/20`、推荐拓扑夹具 `14/14`；正式 `--pure-formal` 的 10k / 50k surface 重放均通过，错侧像素、最长针、冲突覆盖、重复覆盖和 seam 均为 `0`，10k 共识别 `1798` 条水陆直岸且 cell Earcut 未填充数为 `0`。生产构建完成 `1259` modules。
  - Chrome 在 `http://127.0.0.1:5412/` 复用同一地图并定位到用户截图海湾，在 `20 千米` 比例尺执行平滑关 / 开往返：关闭时恢复硬折线，重新开启后岸面连续且两处白针 / 浅色分离带均消失；`http://127.0.0.1:5401/` 新夹具显示底面 / XOR 漂移 `2 → 0`，全量矩阵 `20/20`，两页应用错误均为 `0`。正式 `--pure` 的新增几何断言已通过，仍被既有“关闭平滑时不得保留海岸过渡面”静态源码匹配阻断；第 212 项整体继续执行中，本轮不据局部修复标记完成、提交或推送。
  - 合并 main 后统一复验：实验室继续为 `20/20`、推荐算法 `14/14`；shoreline 完整 `--pure` 与独立 `--pure-formal` 均通过，正式 `10k / 50k` 的错侧像素、最长针、冲突覆盖、重复覆盖和 seam 全部为 `0`。旧静态门禁已改为核对当前 `shoreLayers` 三元表达式中的 `smoothCellBorders` 条件，不再依赖已经移除的 `if` 文本形态。河网、Cells 只读 / 诊断 / 动作、API AI 发现 / 全能力矩阵 / 浏览器能力和生产构建同步通过；API 能力组过期断言由 `16` 更新为运行时真实 `17`。本次只修复回归契约漂移，第 212 项仍保持执行中。
...
第 110～211 项已达到封闭范围内的最小验收；第 195 项阶段 A～D、第 200 项、全游戏复合语义审计、重生成锁定体系、河网校验和 2026-07-26 控制面板 Tab 居中修正均已完成。第 212 项海岸高风险实验矩阵继续执行。海岸专项在并行分支曾使用 `204～209`，本次合并为避开 main 已使用的 `204～206`，统一顺延为 `207～212`；历史提交和开发日志保留旧编号并标注映射。`Q-28～Q-33`、复合语义矩阵中的缺失游戏规则和其它历史缺口没有随本次合并自动转正。
