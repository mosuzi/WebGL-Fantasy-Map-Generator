...
- 政体导出第一刀已完成：`政体管理` 面板新增当前筛选结果的 CSV/JSON 导出，CSV 按国家逐行写出政体、类型、时代、家族、首都、人口、面积、经济力和军力，JSON 同时包含政体分组汇总、国家明细、筛选条件和选中政体上下文。
...
3. 经济/人口的 goods 前移和资源点贸易链路已完成第一刀；后续若继续提质，应补正式供需缺口、市场距离成本、贸易可视化面板，以及让资源货物进一步影响城镇选址和路线权重。
...
7. 外交系统当前已生成关系、附庸、战争历史、管理面板、专题着色、关系矩阵和 CSV/JSON 导出；后续优先补外交导出可读性、贸易偏好展示和关系编辑体验，不再把战争驱动军事行动作为当前路线。
8. 重新生成后的内部状态提示已收敛到 debug 模式；若后续要给普通用户反馈，应改为简短 toast 或状态灯，而不是展示派生系统和调试细节。
9. 政体系统当前已进入生成、经济、外交、军事、国家面板、独立“政体管理”总览、政体专题图层、专题图例、分组批量调整和 CSV/JSON 导出；后续若继续推进，应优先补筛选、编辑可读性和与外交/经济面板的静态交叉展示，政体事件暂不作为当前路线。
...
12. 军事单位编辑面板已完成概要、首屏静态布局、比例面板、工具条分组、态势筛选、手动态势调整、军团重命名、批量态势、驻地/基地、只读战役对象、战斗事件记录、事件列表筛选/清空、事件 JSON/CSV 按范围导出、事件 JSON 导入、轻量结果应用、结果预览明细、持久战报摘要、战报链摘要、链路分组筛选、可点击链路概览、攻防双方损耗摘要、战役对象战报摘要、campaign 轻量双方结算和战役阶段摘要第一刀：当前“军事管理”支持通过二级操作可撤销地修改单个军团态势、名称、驻地、基地，并可选择链路记录、导入、筛选、清空、按筛选清理和按范围导出战斗事件，选择性应用轻量损耗和态势变化；面板首屏会先展示选中军团概要与详情，军团表格作为下方对象切换区；军事生成会提供 `map.military.campaigns` 战役摘要，面板统计、参战军团详情、战役摘要详情和军事 JSON 导出都会带出战役数、战役名、双方兵力、关联 front、共享 `campaign:*` 链路、事件数、已应用数、攻防损耗、战役阶段、优势和进展；已应用 campaign 事件会保存中文结果摘要、兵种损耗摘要、`affectedRegiments`、对手结果和攻守双方损耗分桶，选中军团详情、战斗记录面板标题、战报链摘要和 CSV/JSON 事件导出都会带出共享战役链路、进攻/防守方、对手国家、事件序号、应用状态、损耗状态、攻方/守方损耗分桶和筛选上下文，对手军团也能通过同一条事件看到战报，选中军团事件超过 5 条时可展开完整筛选链路，并可按链路、类型、结果、已应用/未应用筛选。后续军事方向应优先做静态面板观感、导出体验和可读性收尾；除非用户重新要求，不再继续推进动态军事系统、战争行动链路或战役自动结束。
