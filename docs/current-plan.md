...
- **权威任务第 335 项：GPU 常驻视图与零重编译切换。** `进行中；来源：用户要求根治 100k 视图切换端到端卡顿，不能再用 Worker 算法耗时冒充上屏性能`
  ...
  - **阶段状态**：335-A～335-H 已接受并分别以 `0.3.17 / 0.3.18 / 0.3.19 / 0.3.20 / 0.3.21 / 0.3.22 / 0.3.23 / 0.3.24` 建立本地 checkpoint。固定 100k 暖普通颜色为 `101～186ms / 3` 输入包；revision 推进后的 cache 失效为 `5.02s`，session 丢失 cold 为 `21.07s / 1032` 包。B～C 建立稳定 packed surface 与 GPU cell attribute / palette；D～F 让普通 / 政治颜色、主题、海底和普通图层本地提交；G 把平滑边界收敛为 packed correction 与岸线双态缓存；H 把人口、城市、Marker、资源和军事点层改为稳定 GPU ranges，城市总开关只切 draw gate 和轻量 overlay，不再重建 point / picking / overlay 或重装城市实例。固定 `99846` cells 十次反转 / 恢复响应为 `12.2～38.5ms`，固定 `10004` cells 为 `11.1～19.6ms`；两档 Worker、point refresh、overlay replace、picking rebuild、city reinstall 和 LongTask 均为 `0`，引用、picking 语义与最终 framebuffer 精确恢复。当前唯一施工阶段为 335-I。
