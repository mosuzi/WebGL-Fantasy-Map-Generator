...
> **执行门禁（2026-08-19）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-10a` 已由同一只读评审智能体 `ACCEPT`。`349-10b / 0.5.29` 首轮评审 `BLOCK` 删除型 patch 与首府清零两项 P1；`0.5.30` 已补路径值结构门和首府双向唯一引用，修正 checkpoint 待同一智能体复审，未获 `ACCEPT` 不进入 `349-10c`。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，349-7 曾误触发一次带浏览器的既有导出脚本，已首败停止且不计为验收，只在最终阶段形成并评估浏览器验收方案。
...
- 状态：进行中，`349-10b / 0.5.30` 首轮 P1 修正 checkpoint 待复审；`349-0`～`349-10a` 已接受。
