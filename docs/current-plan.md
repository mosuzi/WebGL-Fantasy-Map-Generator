...
- 地图低饱和配色第一刀已完成：默认国家 / 省份色板从高饱和强色块改为灰粉彩色，海面和背景改为灰蓝，海底高度渐变、自然高度色、政体图例、文化 / 宗教 / 政区兜底色、海岸线、政治边界、道路和河流线条同步降饱和。构建产物视觉 smoke 截图为 `docs/generated/screenshots/palette-soft-states.png`，国家色平均 HSL 饱和度为 `0.394`、最高 `0.644`、WebGL error 为 `0`；10k e2e `soft-palette-e2e` 通过，WebGL 加载 `384ms`。
