...
本节是当前唯一有效的执行队列，以当前代码和最新验证结果为准。下面的历史推进记录只作为背景，不再自动转化为授权任务；后续继续推进时，必须以用户明确点名的问题或本节列出的当前优先项为准。

### 阅读规则

- `当前真实状态` 只记录已经由代码、脚本或浏览器验证支持的事实；其中写成“已完成 / 不再作为待办”的内容，不允许在后续执行中重新当作计划项。
- `当前执行队列` 是后续自动推进的唯一清单。每一刀完成后必须同步更新本节并单独提交；浏览器烟测和加载 / 绘制性能检查按风险合并执行，不再每一步默认跑满。
- `可选增强` 只表示未来可以做，不表示当前授权执行；如果用户没有重新点名，不主动从这里取任务。
- `历史记录` 只用于追溯背景和指标，不用于推导下一步。
...
- 简介 tab 已增加保存入口：点击“保存”可通过 dropdown 选择保存到本地文件或浏览器 LocalStorage；两者均使用完整 `webgl-generator-map` 文档，包含 map 数据、typed arrays 和 options。浏览器保存会优先使用 gzip+base64 压缩后写入 LocalStorage；下次打开页面若存在有效浏览器存档，会跳过随机生成并直接恢复该地图，恢复失败会清理损坏存档并回退到正常生成。
...
- 地图 DOM overlay 临时隐藏策略已按用户要求取消：拖动、缩放、适配视图和定位对象时不再给 `.map-stage` / `#map-overlay` 挂交互隐藏类，`drawViewportPreview()` 会同步刷新标签、城市剪影、marker 和军事图标位置，让非 canvas 覆盖层与 canvas 一起移动 / 缩放。此前隐藏策略的 profiling 仅保留为历史对照，不再代表当前行为。
...
- viewport idle commit 分帧仍保留，但只延后路线、河流和选中态等 screen-space 动态 mesh 重建；DOM overlay 不再延后或隐藏。Vite dev 浏览器烟测覆盖滚轮、左键拖拽、适配视图和对象定位，四类路径中 `map-stage--interaction-hidden = false`、`map-overlay--interaction-hidden = false`、`#map-overlay` 保持 `visible`；左键拖拽后 city label / city icon 屏幕位移约 `158.8px`，定位对象后 city label 位移约 `162.6px`，确认覆盖层随相机同步更新。100k `viewport-overlay-sync-100k` 完整图层滚轮 / 中键拖拽 frame p95 为 `70.6ms / 35.4ms`，overlay p95 为 `5.8ms / 2.4ms`，overlay 暂停样本为 `0`，idle dirty 恢复 `clean`；10k e2e `viewport-overlay-sync-e2e` 通过，WebGL 加载 `405.3ms`。
...
- 面板工具按钮组审计已区分文字按钮和图标按钮：`floating-panel-header-actions` 与 `UiPanelIoActions` 等预期小图标按钮不再套用 `72px` 文字按钮阈值，但仍检查横向溢出和最小图标尺寸；`panel-space-next / continents / 10000 / deep` 审计未发现待复核项，面板预热 `18 / 18`，点击到出图 `1358.4ms`、WebGL 加载 `341.7ms`、`glError = 0`。
...
- 军事管理顶部筛选区已拆行：`国家 / 态势` 两个下拉保留第一行，搜索筛选框跨整行显示，避免三项硬挤在列表上方同一行。
- 军事兵种比例弹框已改进：通用 `UiActionDock` 二级浮层支持拖动标题栏，兵种比例动作声明更宽 / 更高的预估尺寸；比例弹框改为两列紧凑网格。构建产物烟测确认兵种比例弹框初始 `620 x 619px`、内容区 `scrollDebt = 0`、`5` 个兵种为 `2` 列 `3` 行，拖动后位置变化 `23px / 70px`，`glError = 0`。
- 列表 ID 排序已统一为数字优先：新增 `ui/sort-utils.js`，所有主要 `UiObjectTable` 面板的 `sortRows` 入口改为先尝试纯数字比较，再回退中文字符串比较；ID 平局回退也不再用字符串 `localeCompare`。脚本验证确认 `['1','2','9','10','11']` 升序正确，行 ID 升序为 `2,9,10`、降序为 `10,9,2`；`pnpm run build:app` 通过。
- 编辑面板导入 / 导出入口已改为小图标动作条：新增 `UiPanelIoActions`，导出统一通过暗色 dropdown 选择具体格式，导入通过同尺寸小图标触发文件选择；经济、政体、外交、军事、测量、备注、名称库和高度图导入工作台不再把导入 / 导出做成列表上方或工作台底部的大按钮。构建产物烟测确认 9 个动作条均存在，按钮尺寸均为 `32 x 32`，旧大导出 / 导入入口计数为 `0`，经济导出 dropdown 背景为 `rgba(12, 18, 22, 0.98)`，无面板横向溢出，`glError = 0`。
...
- Element Plus 暗色主题变量第一刀已完成：按官方主题文档整理为集中 CSS 变量层，并把项目 `styles.css` 改为入口导入，降低本地 dev 与构建产物的样式顺序差异；二级编辑面板关闭按钮已脱离 Element text button，避免 hover 白色椭圆。当前项目未安装 `sass`，所以还没有切到官方 SCSS source theme；完整 `@forward ... var.scss` + `importStyle: "sass"` / `useSource` 仅作为可选后续。
- 对象颜色编辑已支持 HSL 取色：共享 `UiColorField` 不再依赖 Element Plus `ElColorPicker`，改为项目专用 HSL 字段，包含色块、H/S/L 滑杆、H/S/L 数字输入、HEX 输入和预设色；对外仍输出标准 `#rrggbb`，国家 / 省份 / 文化 / 宗教颜色命令和地图数据格式不变。构建产物烟测确认国家颜色二级面板可用 HSL `210/45/62` 应用为 `#729eca`，历史记录为 `国家颜色 #1`，面板无横向溢出，WebGL error 为 `0`；10k e2e `hsl-color-field-e2e` 通过，WebGL 加载 `396.6ms`。
- 列表型面板选中滚动已改为居中：共享 `UiObjectTable` 不再只把选中行滚到刚好可见，而是让选中行中心对齐滚动容器中心；靠近顶部或底部无法居中时停在边界。构建产物烟测在城市管理 821 行中选中靠后行，选中行中心与容器中心差值约 `-0.31px`，`glError = 0`，console/page error 为空。
...
- 战报记录筛选区已拆成两行：`链路 / 类型 / 结果` 第一行各占 1/3，`结算 / 导出` 第二行各占 1/2，避免五个筛选项挤在同一行导致内容展示不完整。构建产物样式烟测在 `620px` 宽容器中确认筛选区为 `2` 行，第一行项宽约 `202px`，第二行项宽约 `306.5px`，无横向溢出，`glError = 0`。
...
- overlay profile 事件处理探针已补齐：`profile:overlay` 会在浏览器内记录 wheel / pointer 事件的同步 dispatch 耗时、到下一帧延迟、默认阻止数量和完成阶段，并写入 JSON 与 Markdown 报告。100k `overlay-event-probe-100k / full` 通过，滚轮 / 拖拽 dispatch p95 均为 `0.1ms`，说明当前瓶颈不在同步事件回调本身；滚轮到下一帧 p95 约 `127.5ms`，idle commit 中 route / river build 约 `93.6ms / 67.9ms`，后续性能专项应继续优先拆 idle commit 或帧调度压力。
- viewport idle commit 切片收细第一刀已完成：路线 / 河流 screen-space mesh 异步重建 slice 从 `10ms` 收到 `5ms`，减少停止输入后单帧占用。100k 同 seed `overlay-event-probe-100k / full` 对照中，滚轮 / 拖拽 idle commit 帧 p95 从上一轮记录约 `47.1ms / 23.6ms` 降到 `17.7ms / 11.9ms`，idle long task 为 `0`，dirty 均恢复 clean；构建产物 profile 仍通过，滚轮 / 拖拽 dispatch p95 为 `0.1ms`。
- viewport idle commit 过期取消已加固：路线 / 河流异步 screen-space mesh 构建会在开始时捕获相机快照，路线同时复制 selection 快照；每次分片 yield 返回后立即检查版本 token，过期任务标记 `aborted` 并跳过 buffer 上传，避免连续滚轮 / 拖拽后的旧构建结果覆盖新视口。`pnpm run build:app` 通过；本刀不跑重 profile，留到后续性能专项合并验证。
- 地图 DOM overlay 定位已改为 transform 路径：城市 / 国家 / 手工标签、城市图标、marker 图标、军事图标和选中 marker 不再在每帧写 `left/top`，改写 `--overlay-x / --overlay-y` 并由 CSS transform 合成定位；图标 transform transition 已移除，避免移动缩放时出现尾随。100k 同 seed `overlay-event-probe-100k / full` 中滚轮 / 拖拽 frame p95 为 `47.2ms / 35.4ms`，较前一刀同 seed `100ms / 52.8ms` 下降，长任务从 `10 / 4` 降到 `2 / 0`；浏览器 smoke 确认可见 overlay `left/top = 0` 且 bounding box 正常，`glError = 0`。
- GEO 数据导入已分流并修正水陆语义同步：普通 GeoJSON `FeatureCollection / Feature / Geometry` 仍按当前地图 `mapCoordinates` 反投影为可撤销测量对象；原版 FMG `Cells` GeoJSON 会按 `properties.height` 识别为地形来源，反算原版海拔米数到当前 `0..100` 高度单位，并按源 cell polygon 中心重采样到当前 grid / pack 高度。导入或撤销后会重建 grid feature、既有 pack 图上的 feature / haven / harbor、climate biome / population 和渲染缓存，避免“颜色是陆地但悬停显示海洋”或反向错位；悬停 picking 也已按 pack/grid 各自 feature 来源读取。真实文件 `k170 Cells 2026-07-04-22-00.geojson` 浏览器回归确认 FMG Cells 地形导入后 `grid mismatch 0`、`pack mismatch 0`、`hover mismatch 0 / 80`，WebGL 加载 `427ms`；当前只导入地形高度，不导入国家 / 城市 / 文化 / 宗教等语义层。
...
- 国家编辑面板已补集合级新增 / 删除第一刀：列表下方新增小图标动作条，新增模式会等待下一次点击陆地 cell，以该 cell 对应 pack cell 创建新国家、默认省份和首都，并接入统一 `EditHistory`；删除选中国家会把其 cell 归为中立、移除省份归属并保留为可撤销命令。标签管理的“新增标签 / 删除标签”也已从大文本按钮收敛为同类小图标动作条。
- 省份编辑面板已补集合级新增 / 删除第一刀：列表下方新增小图标动作条，新增模式会等待下一次点击已有国家内的陆地 cell，以该 cell 作为中心创建省份并把点击 cell 及同国相邻陆地 cell 归入新省份；删除选中省份会清空省份归属但不删除国家边界或城市对象，并接入统一 `EditHistory`。
- 国家 / 省份列表的编辑链路已去重：对象操作栏里的“进入编辑”图标现在是直接开关，不再打开只包含“编辑此国家 / 编辑此省份”的二级浮层；图标会在编辑态高亮，再次点击会关闭编辑态。原底部“启用国家编辑 / 启用省份编辑”按钮已移除，目标选择、取选中、取悬停和半径控制保留。
- 城市管理面板已补集合级新增 / 删除第一刀：列表下方新增小图标动作条，新增模式会等待下一次点击没有既有 burg 的陆地 cell 并创建城市 / burg；删除选中城市使用 `removed` 标记和 pack cell burg 索引清理，不移动其它城市 ID，并会修复首都 / 省会引用。
...
下面是继续推进时的真实顺序。每一刀完成后单独提交；语法 / 构建验证保持就近执行，浏览器烟测和加载性能检查可以隔几步合并，除非改动本身明显触及渲染、生成或交互主链路。
...
- Element Plus 完整 source theme：安装 `sass`，新增项目级 `styles/element/index.scss` 用 `@forward 'element-plus/theme-chalk/src/common/var.scss' with (...)` 承载主题 token，并把 Vite 按需样式切到 `ElementPlusResolver({importStyle: "sass"})` 或 `unplugin-element-plus useSource`。这会更接近官方“全量主题替换”路线，但需要单独评估构建耗时和 CSS chunk 体积。
...
- 编辑面板新增 / 删除统一化：国家、省份和城市面板已完成第一刀；后续可按具体面板逐个补齐新增 / 删除命令，并统一放在列表下方小图标动作条中，避免再出现醒目的大文本按钮。文化、宗教、路线、河流、湖泊、备注、名称库、测量对象和资源标记等面板应按各自数据约束逐项处理，不能用一套粗暴删除逻辑横扫。
...
- 不把 `docs/current-plan.md` 的历史记录、旧编号条目或专题 backlog 当成自动授权；每一步仍需按用户明确问题推进，完成后单独提交，烟测与加载 / 绘制性能检查按风险合并执行。
...
- K170 原版城镇名已加入内置备选库：从用户原版 FMG 导出的 Burgs CSV 提取 `Burg` 列，注册为 `k170-burg-names / K170 原版城镇名`，类型为 `place`。共 `1086` 条去重样本、加权总量 `1129`，保留重复出现的城镇名权重；该库只作为后续地名绑定 / 显式重命名候选，不自动覆盖当前地图已有城镇名。
- 文化面板枚举显示已中文化：文化类型列表和详情不再直接显示 `Generic / Naval / Lake / River / Nomadic / Hunting / Highland / Desert` 等内部枚举，命名风格也不再显示 `default`，改为“通用 / 海洋 / 湖泽 / 河流 / 游牧 / 林猎 / 高地 / 沙漠”和“默认 / 欧式”。筛选仍保留内部 key 和中文标签双通道匹配。
- 可撤销面板的历史入口已移到 header：`PanelManager` 统一在关闭按钮左侧渲染撤销 / 重做 icon 按钮，按钮读取各面板共享的 `EditHistory` 统计并按可用状态禁用；原来面板底部的 `UiHistoryActions` / 高度编辑历史条已隐藏，避免底部操作难以触达和重复出现。政体管理批量政体调整也补上同一套 header 撤销 / 重做入口。
...
3. 经济/人口的 goods 前移和资源点贸易链路已完成第一刀；经济总览已完成只读面板、商品/市场/交易筛选排序、定位、当前筛选结果 CSV/JSON 导出、debug-only 开发诊断、市场距离成本、商品供需缺口、资源货物路线权重、资源货物城镇选址、静态价格传播诊断、贸易流价格信号和市场归属诊断第一刀。贸易流地图连线已按用户校准退役，后续贸易信息优先走经济面板或独立列表面板按国家、地区、商品筛选展示，不再默认做同时铺满地图的连线可视化；贸易动画继续后置。
...
258. 湖泊管理面板第一刀已完成：管理 tab 新增“湖泊管理”入口，面板支持湖泊总览、筛选、排序、选中、定位、详情、单湖泊重命名、撤销/重做，以及“按名称库重命名筛选”批量入口；批量命令读取当前 `hydro` 名称库，只写湖泊 feature 名称，不重建水文、河网、路线、政治或军事数据。构建产物浏览器烟测中注入 `hydro = lake-smoke-hydro` 测试库，5 个湖泊按筛选结果显式重命名，撤销恢复原名、重做恢复新名，湖泊面板横向溢出为 `0`，`glError = 0`，health 非 info、console error、page error 均为 `0`。正式 e2e 守门通过：`lake-panel-smoke / continents / 10000` 点击到出图 `1652.4ms`，纯生成 `844.4ms`，WebGL 加载 `423.6ms`，UI/调度余量 `384.4ms`，最慢加载阶段为“构建视觉 cell mesh” `61.4ms`，结果仍在守门阈值内。用户再次校准：无需做动态军事系统，后续军事方向只保留静态面板、态势线观感、军团展示和导出可读性收尾。

259. 测量对象保存第一刀和控制面板布局修正已完成：测量 readout 新增“保存 / 对象”，可把当前临时测量保存到 `map.measurements.items`，生成 id、名称、点列、长度/面积摘要并清空临时点；新增懒加载“测量对象”浮层，支持总览、筛选、排序、定位、重命名、删除、导出、撤销/重做，完整地图 JSON 会保留 `map.measurements`。同时修正控制面板“图层 / 管理 / 重新生成”按钮网格中 Element Plus 相邻按钮 margin 导致的第二行错位，并收束 `UiSliderField` 数字输入上下调节按钮的白色边线。构建产物烟测中图层页 8 行、管理页 10 行首列 left 均为 `365`，三点测量保存为 `测量 1` 后导出、删除、撤销均正常，数字调节按钮边线为深色，`glError = 0`。正式 e2e 守门通过：`measurement-object-smoke / continents / 10000` 点击到出图 `1544.4ms`，纯生成 `684.4ms`，WebGL 加载 `498.4ms`，UI/调度余量 `361.6ms`，最慢加载阶段为“构建视觉 cell mesh” `63.9ms`，`line-vertices = 47.8ms`，`drawMs = 0.1ms`，`glError = 0`。后续测量方向继续做保存对象图层化、已保存对象节点编辑和路线贴合。
260. 保存测量对象图层化第一刀已完成：控制面板图层页新增“测量”图层，renderer 默认保存 `measurements = true`；运行时在原测量 SVG overlay 中绘制已保存对象，保存对象用蓝色虚线和淡色面片，临时测量仍保留金色可编辑点。退出测量工具后，若“测量”图层开启且存在保存对象，overlay 只显示对象图形并隐藏 readout；关闭“测量”图层时对象图形消失但 `map.measurements.items` 不变。构建产物浏览器烟测中保存三点测量后退出工具，`.measurement-object-path = 1`、`.measurement-object-area = 1`；关闭图层后 `stored = 1`、`overlayHidden = true`，重新打开后对象恢复且 `glError = 0`。正式 e2e 守门通过：`measurement-layer-smoke / continents / 10000` 点击到出图 `1312.3ms`，纯生成 `714.4ms`，WebGL 加载 `373ms`，UI/调度余量 `224.9ms`，最慢加载阶段为“构建线层顶点” `75.4ms`，`drawMs = 0.1ms`，`glError = 0`。后续测量方向继续做已保存对象节点编辑和路线贴合。
261. 已保存测量对象节点编辑第一刀已完成：测量对象面板新增“编辑形状”，会把选中对象点列复制进测量工具编辑缓冲，开启测量模式并隐藏该对象的只读蓝色旧形状；用户可复用当前临时测量节点拖拽、删除和线段插入能力，点击“保存修改”后通过 `createUpdateMeasurementPointsCommand()` 写回原对象、更新摘要和 `updatedAt`，并接入 `EditHistory` 撤销/重做。构建产物浏览器烟测中三点对象首点从 `{x:489.6,y:336}` 拖到 `{x:505.973,y:342.466}` 后保存，`updatedAt` 变化，撤销后首点恢复到 `{x:489.6,y:336}`，`glError = 0`。正式 e2e 守门通过：`measurement-edit-smoke / continents / 10000` 点击到出图 `1552.3ms`，纯生成 `765.8ms`，WebGL 加载 `500ms`，UI/调度余量 `286.5ms`，最慢加载阶段为“构建视觉 cell mesh” `76ms`，`line-vertices = 69.9ms`，`drawMs = 0ms`，`glError = 0`。后续测量方向继续做路线贴合、完整导入回归烟测和曲线尺细化。
...
269. 地区纹理图层第一刀已完成：复用已有 `zones` 数据作为“地区”图层，控制面板新增“地区”开关，renderer 新增 WebGL 纹理绘制层，支持斜线、交叉线和圆点阵列三类基础纹理；国家和省份默认颜色仍保持纯色，不参与纹理。生成端降低默认地区密度到 `2-6` 个，新增 `Warzone` 类型，优先从实际军事 front / Enemy 边界附近生成少量战区；已有大型宗教战区范围被压缩，避免满图纹理。地区导出补充 `pattern / hexColor`。后续待做：手动创建地区。

270. hover 人口调试行已补充：cell hover 增加“调试”行，展示 pack 侧 `biome / height / suitability / packCell / flux / resource`，用于定位温度降水看似可居住但人口为 `0` 的具体原因。该信息只读展示，不改变人口公式。

271. 对象动作区操作收敛已完成：国家和省份面板的“新增 / 删除 / 编辑”现在统一放回原编辑按钮所在的对象动作区，并保持新增在第一位；编辑按钮直接切换编辑态并高亮。城市新增/删除也并入城市对象动作区。测量对象的编辑、定位、删除收敛为列表小图标，河流编辑从二级弹层里的大按钮改为直接图标切换。

272. 国家/省份编辑按钮状态链路修复已完成：国家和省份的编辑按钮现在进入编辑时会同步写入全局 `editingObject`，退出、切到新增或被其他编辑器关闭时会清理对应编辑对象；外部 `setActive(false)` 也会走同一套退出逻辑，避免按钮看起来点了但运行时状态没有变化。

273. 国家/省份对象动作按钮运行时异常已修复：国家和省份面板中“新增 / 删除 / 编辑”的 Vue handler 不再引用裸 `callbacks`，统一改为 `props.callbacks`，避免点击按钮时抛 `ReferenceError` 导致看起来完全无反应。

274. 新增/删除临时态互斥与关闭清理已完成：国家、省份、城市对象动作区新增独立删除态；新增态和删除态都会让当前按钮常亮、同区域其他结构性按钮置灰，并支持再次点击退出。进入新增/删除态会清理同面板编辑态和其他互斥编辑器；关闭整个面板会清除新增、删除、编辑等临时状态。构建产物 Playwright 定向验证覆盖国家、省份、城市三类面板的新增/删除按钮常亮、互斥置灰、再次点击退出，以及关闭面板后 `activeEditor` 清空。

275. 中立地区新增国家语义已修正：新增国家明确允许从 0 人口中立陆地 cell 创建，首都会按最少人口兜底生成，不再把人口或适居度作为用户直接输入的阻挡条件。点击中立 cell 时，新国家只吸收同为中立的相邻陆地 cell，避免旧逻辑把周围已有国家的邻居格子一并吞掉；pack 首都 cell 选择也优先匹配点击 grid cell、原归属和同 feature，缺少直接映射时才找最近陆地 pack cell 兜底。

276. 地区管理面板第一刀已完成：管理 tab 新增“地区管理”，面板展示地区纹理图例、地区列表、类型中文名、纹理名、涉及国家、面积和规模；选中地区会进入 `zone` 对象语义并在地图上半透明高亮，定位按钮会缩放到该地区范围。该面板先解决“图上的纹理分别代表什么”的可读性问题，暂不做手动创建和编辑。
...
279. 地图外缘与气候范围锁定回修已完成：issue #1 上一刀仅在地图内部叠加弱渐变，仍会让画布外底色和地图边缘形成锯齿式分割；当前改为跨世界边界的内外双向 feather，并把 `.map-stage` 背景同步为地图未开发底色，让缩放后世界边界内外颜色连续过渡。气候范围锁按钮改为跨“纬度范围 / 经度范围”两条滑杆右侧两行垂直居中，并增加两条连接线；默认锁定状态写入全局偏好默认值，用户手动解锁后会保存到 `webgl-generator-control-preferences`，后续打开保持用户选择。

280. 地区纹理可见性已修正：地图地区层不再绘制实心半透明底色，只用纹理线 / 点表达区域范围；斜线、交叉线和圆点阵列改为每个 cell 内多线 / 多点绘制，并把大地区的纹理抽样上限从 `220` 提到 `720`，避免远看像实心色块或纹理过疏。地区管理面板图例的斜线、交叉线和圆点也同步加密，与地图表现一致。构建通过，并用构建产物截图确认入侵区在地图上显示为斜线纹理而不是实心高亮。

281. 地区样式编辑入口已完成：地区管理面板新增对象动作区“调整样式”，可用共享 HSL 取色面板修改 `hexColor`，并可在斜线 / 交叉线 / 圆点阵列之间切换 `pattern`；命令接入 `EditHistory`，只刷新地区线层、选中态和对象面板，不触发政治、军事或经济派生重算。构建产物浏览器烟测中地区 `#0` 从 `#d98238 / diagonal` 改为 `#7f6cc7 / dots`，撤销恢复纹理为 `diagonal`，重做恢复 `dots`，`glError = 0`、console/page error 为 `0`。后续地区方向继续做手动创建地区。
