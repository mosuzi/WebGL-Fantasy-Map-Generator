...
> **执行门禁（2026-08-19）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-10c` 已由同一只读评审智能体 `ACCEPT`。`349-10d` 两轮评审共发现七项 P1，第二轮剩余的 Feature source identity 与 river 逐段 owner 已在 `0.5.38` blocker-only 候选中修正并完成纯 Node 复验，正等待同一评审智能体复审；未获 `ACCEPT` 不进入 349-10e。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，349-7 与 349-10c 各误触一次命名不显式的浏览器脚本，均已立即停止且不计验收，最终阶段只形成并评估浏览器验收方案与防误触门。
...
- 状态：进行中，`349-10d / 0.5.38` 第二轮 blocker-only checkpoint 待复审；`349-0`～`349-10c` 已接受。
...
- 阶段：`349-0` 计划冻结；`349-1` 现状盘点；`349-2` TS 工具链；`349-3` 核心契约；`349-3a` 字段注册表与身份适配器闭合；`349-4` Manifest；`349-5` 薄 facade；`349-6` notes 切片；`349-7` markers 切片；`349-8` Worker 切片；`349-9` dependency / projection；`349-10a / 10b` 已接受；动态插入 `349-10c0` 共享 Worker result ownership；随后 `349-10c`～`349-10g` 依赖序逐域迁移与旧路径收口；`349-11` 非浏览器终验与浏览器验收方案。
