...
控制台 / 扩展 API 系统的权威任务第 28～34 项、依赖稳定 API 的第 35 项快捷键机制，以及第 36～44 项后续能力已经全部完成并移出活动清单。下一波第 45～52、54 项已经完成范围整理，但按用户 2026-07-15 最新指令暂停实施。现有功能与界面信息架构审计已完成，审计结论正式提升为权威任务第 55～60 项；原第 53 项已按用户决定移除，短期不深化纹章系统。
...
> **暂停门禁（2026-07-15）**：用户明确要求先不实施第 45～52、54 项。现有功能与界面信息架构审计结论已写入 [`docs/audits/ui-function-and-information-architecture-audit.md`](./audits/ui-function-and-information-architecture-audit.md)，并按用户最新指令提升为第 55～60 项权威任务。第 55～60 项当前只完成范围冻结，不在本次军事纠偏中顺带实施；后续开工时先按第 55～60 项的 UI 整改顺序执行，再恢复第 45～52、54 项。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、168 个公开方法和 78 个编辑方法；第 28～44 项均已达到各自封闭范围内的最小验收并移出活动清单。

下一波继续采用目标模式：只实现当前编号项的封闭范围，达到最小验收后立即转入下一项；不影响当前验收的新发现只写入 `FOLLOWUPS.md`。日常实现以代码检查和专项回归为主，只有验收明确要求时才运行浏览器。

- **权威任务第 55 项：管理入口分组与首层 / 高级信息架构。** `待执行；来源：UIA-01、03、05、06、13`
   - 范围：把管理入口按“地形环境、政治社会、网络经济、标注对象、系统工具、实验功能”分组；把“适配视图”移回地图导航；纹章退出管理首层但保留数据、渲染和导入导出兼容；生成首层只保留 seed、规模、尺寸、模板和生成，高级气候默认收起；普通界面优先使用中文业务术语。
   - 验收：管理页不再平铺 25 个同权入口，六个领域组均有稳定标题和首层入口；“适配视图”只保留一个地图导航入口且快捷键仍调用同一 action；纹章只在实验 / 更多层可达；高级气候默认收起且展开后能力无损；入口清单回归、生产构建和一次真实浏览器查找路径抽查通过。

- **权威任务第 56 项：主面板共存、恢复与统一浮层层级。** `待执行；来源：UIA-02、11、14`
   - 范围：确立“一个主编辑面板 + 可选对象详情”的默认共存规则；打开新主面板时替换旧主面板或进入明确停靠区；持久化只恢复最后一个主面板；导出、彩色高度图工作台、树状显示等固定高级浮层接入统一 overlay registry，并共享层级、焦点、关闭和地图安全区规则。
   - 验收：`1280×720`、窄视口和持久化恢复样本中，主面板不再默认互相覆盖，固定浮层不会遮住关闭控件、底部提示或必要地图工具；Esc、关闭、焦点返回和层级切换一致；多面板碰撞 / 恢复专项审计、生产构建和浏览器矩阵通过。

- **权威任务第 57 项：重复入口、动作分组与控件词表收口。** `待执行；来源：UIA-04、10、12及破坏性动作审计`
   - 范围：对象表格统一逐行定位，移除无特殊价值的底部“定位选中”和重复双击定位；领域重生成优先回到对应领域面板，控制面板只保留整图生成和一个带影响摘要的统一重生成入口；同类操作组成稳定功能区；统一关闭按钮和最小图标词表，并把删除、替换、批量覆盖等破坏性动作与安全动作视觉分离。
   - 验收：入口盘点中的重复定位和双重领域重生成均按规则归零或有明确例外记录；保留入口与快捷键 / API 共用同一 action；相关功能位于同一动作区；关闭和常用动作图标具有统一组件与可访问名称；专项静态回归、生产构建和浏览器抽查通过。

- **权威任务第 58 项：高度、军事与导出高复杂度面板分层。** `待执行；来源：UIA-07、08、09`
   - 范围：高度首层保留画笔、常用选区和导入，数学变换、地形程序、色板诊断和 GPU 统计降为高级；军事首层保留军团列表、定位、基础态势、兵种比例和重生成，战报档案、链路筛选和轻结算降为高级 / 试验；导出拆为快速导出和按格式展开的高级选项。
   - 验收：三个面板首次打开只展示各自首层核心闭环，高级能力默认收起但全部仍可达；快速 PNG / 地图 JSON 保持一步可执行；既有编辑、导入导出、撤销和 API 语义不变；面板能力清单、专项回归、生产构建和三个真实浏览器路径通过。

- **权威任务第 59 项：对象术语、交互状态与反馈语义统一。** `待执行；来源：补充审计第 1～3、7～8 项`
   - 范围：统一“资源点 / 通用标记”“水体 / 湖泊 / feature”“测量 / 测量对象”等对象边界；统一选中、编辑、预览、待派生、空数据、无筛选结果、孤儿对象和操作失败的视觉与文案；高频程度决定入口深度，不再按功能完成时间排列。
   - 验收：术语表覆盖主要面板、控制面板和提示文案且同一对象不再跨入口换名；选中 / 编辑 / 预览具有可区分且可退出的共享状态；空态、异常态和待派生状态均提供稳定恢复动作；文案 / 状态专项回归、生产构建和浏览器状态抽查通过。

- **权威任务第 60 项：多面板、可访问性与挂载性能审计门禁。** `待执行；来源：UIA-15及补充审计第 4～6 项`
   - 范围：扩展现有单面板布局审计，覆盖多面板组合、持久化恢复、固定浮层、浏览器缩放、系统字体放大、底部提示安全区、焦点进入 / 返回、Esc、Tab 顺序、图标按钮名称、快捷键冲突，以及大型低频面板的懒加载、mount 长任务和连续打开内存变化。
   - 验收：审计工具能稳定复现并报告上述矩阵，不再用“单面板无溢出”代替整体合格；核心路径可仅用键盘完成打开、操作和关闭；窄视口 / 缩放样本无关键控件不可达；大型低频面板具备可核验的懒加载和挂载性能报告；生产构建与一次完整浏览器审计通过。

- **权威任务第 45 项：曲线尺连续采样与平滑闭合。** `待执行；来源：measurement-rulers-plan`
   - 范围：在既有自由折线、面积、路线贴合和持久测量对象之上，增加拖拽连续采样、稳定点列简化和可选平滑闭合；UI 与 `edit.measurements.*` 共用对象参数和命令，继续使用结构化 JSON 和本地 `EditHistory`，不复刻 SVG `getTotalLength()`。
   - 验收：同一路径在固定采样参数下得到稳定点列；简化前后长度/面积误差在明确阈值内；自由、贴路和曲线对象可区分并完整往返；编辑、撤销/重做、图层显隐和导出一致。专项几何回归、测量往返回归、生产构建和一次真实浏览器拖拽抽查通过。

- **权威任务第 46 项：备注独立导入与孤儿批量治理。** `待执行；来源：object-notes-implementation-plan`
   - 范围：为现有备注摘要文档增加预检后的追加/替换导入，并接入 `edit.notes.import / deleteBatch`；备注总览支持只选择孤儿、批量导出和单事务删除。纯文本契约保持不变，不引入富文本、Markdown 或 AI。
   - 验收：当前摘要导出可无损导回；重复 id、缺失对象、坏版本和混合有效/无效记录给出结构化预检；取消或失败不改地图；批量删除只形成一条历史并可完整撤销/重做；旧地图备注兼容。专项回归和生产构建通过。

- **权威任务第 47 项：名称库多词率与剩余显式入口。** `待执行；来源：namebase-editor-plan`
   - 范围：让原版文本字段 `m` 进入项目自身的多词生成行为，并补齐仍有稳定名称身份的对象面板显式重命名入口；UI 继续复用 `namebases` API 与统一批量命令，导入、绑定和编辑名称库仍不得自动改写已有对象。
   - 验收：固定 seed 与固定 `m` 得到可复现的多词比例和分隔行为；`m=0` 保持当前单词兼容；JSON、原版文本、完整地图和本地偏好往返保留参数；显式批量重命名进入一条历史并可撤销。名称库、生成绑定、旧数据与 API 回归及生产构建通过。

- **权威任务第 48 项：市场归属编辑与经济链内重算。** `待执行；来源：economy-market-trade-plan`
   - 范围：基于既有市场归属诊断增加市场 cell 归属笔刷和预览/应用/取消，并新增对应 `edit.economy` 与经济重算 API；只闭合经济链，不驱动政治、外交或军事。
   - 验收：取消不改变 `pack.cells.market`；应用形成一条历史，撤销/重做恢复市场归属、生产、交易、价格压力和财政摘要；跨国/无效覆盖在应用前明确提示；面板、静态贸易流与 CSV/JSON 同步。专项回归、API 门禁、生产构建和一次浏览器笔刷闭环通过。

- **权威任务第 49 项：路线改线、端点重连与样式编辑。** `待执行；来源：editor-and-stat-panel-inventory`
   - 范围：在既有路线创建/删除之上支持单路线几何改线、城市端点重连以及 `type / level` 样式调整；UI 与新增 `edit.routes` 方法共用命令，继续遵守陆路/水路约束和路线拓扑，不扩展动态交通模拟。
   - 验收：非法穿越、断点和不存在端点明确拒绝；有效改线同步 cells、距离、资源诊断、picking、面板和 GeoJSON；单次应用只形成一条历史且撤销/重做完整；相关经济派生按约定标脏或重算。专项拓扑/导出/API 回归、生产构建和一次浏览器改线抽查通过。

- **权威任务第 50 项：水体与 feature 局部修正。** `待执行；来源：editor-and-stat-panel-inventory`
   - 范围：补湖泊出口编辑和有预检的局部陆水/feature 修正，UI 与 `edit.lakes / edit.features` 共用命令，维护 grid/pack 高度、feature、岸线、haven/harbor 与河流引用；不做整图侵蚀或完整海岸雕刻器。
   - 验收：开放海域、无效出口、城市/路线/河道冲突和过大范围明确拒绝；预览、应用、取消、撤销/重做生命周期完整；湖泊进出口、岸线、feature 引用和水文 stale 状态一致；保存往返和旧地图兼容。专项水文/feature 回归与生产构建通过。

- **权威任务第 51 项：生物群系与适居度局部编辑。** `待执行；来源：editor-and-stat-panel-inventory`
   - 范围：为生物群系面板增加目标 biome、范围和半径笔刷，并通过 `edit.biomes.assignCells` 暴露同一命令；按当前气候/高度约束提示异常，应用后重算适居度与人口承载相关摘要，只把城镇及下游系统标记为待派生。
   - 验收：笔划预览不写历史，抬手只提交一条命令；陆水不匹配和越界目标拒绝；grid/pack biome、统计、专题色、适居度与导出一致；撤销/重做及旧地图往返稳定。专项回归、生产构建和一次浏览器笔刷抽查通过。

- **权威任务第 52 项：用户主题导入导出与颜色级编辑。** `待执行；来源：visual-theme-preset-plan`
   - 范围：在六个内置主题之上增加白名单 JSON 导入导出、用户主题持久化和陆地/水域/边界/道路/主要标签/比例尺颜色编辑；UI 与 `layers` 主题 API 共用 registry 和校验，不做纹理、字体系统或高级后处理。
   - 验收：坏 JSON、未知 token 和越界颜色不改变当前主题；用户主题可选择、编辑、撤销/重做，并随完整地图及旧数据迁移往返；renderer、DOM overlay、图例和 PNG 使用同一 token。主题回归、PNG 回归、生产构建和一次浏览器切换/编辑抽查通过。

- **权威任务第 54 项：GeoJSON 范围导出与坐标参考强化。** `待执行；来源：export-capability-matrix`
   - 范围：pack cell 与要素 GeoJSON 支持当前视口或显式世界坐标 bbox 范围；采用“导出与范围相交的完整 feature”语义，不在本项裁切几何；补充可机读的地图世界边界、范围和近似等距圆柱坐标参考说明，不伪造 EPSG。
   - 验收：全图默认输出保持兼容；同一 bbox 下 pack/feature 图层筛选稳定，集合与 feature bbox 正确，空范围/越界拒绝；普通与 dissolve 政治面、100k 性能和 QGIS 读取继续通过；UI 与 API 共用同一范围参数。专项范围/API/GIS 回归、生产构建和真实浏览器文件级抽查通过。

第 55～60 项已经由用户批准写入权威清单，但本次只实施军事重生成纠偏，不顺带启动 UI 整改。第 45～52、54 项继续暂停；第 53 项已移除且不作为隐含待办。下一波 UI 开工时按第 55～60 项顺序执行；全部达到最小验收后再恢复第 45～52、54 项，不得默认直接从第 45 项开工。
