...
- **权威任务第 335 项：GPU 常驻视图与零重编译切换。** `进行中；来源：用户要求根治 100k 视图切换端到端卡顿，不能再用 Worker 算法耗时冒充上屏性能`
  ...
  - **阶段状态**：335-A 已接受并以 `0.3.17` 建立本地 checkpoint。固定 100k 暖普通颜色为 `101～186ms / 3` 输入包；正式 revision 推进后虽仍为 `3` 包，却因渲染 cache 整体失效耗时 `5.02s`；session 丢失后 cold 为 `21.07s / 1032` 包，其中输入复制 `12.21s`。10k / 100k LongTask、应用错误、Loading 与 WebGL error 均为 `0`。A 阶段另修正 emergency fan cell 被提到几何数组前端后颜色补丁 ID 非递增、导致 100k 首次切换被拒绝的交叉缺陷；几何绘制顺序未改。当前唯一施工阶段为 335-B。
