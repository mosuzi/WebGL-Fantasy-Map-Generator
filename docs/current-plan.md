...
当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、188 个公开方法和 90 个编辑方法，稳定等级为 `180 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。
...
- **权威任务第 73 项：城市移动、迁国迁省及港口路线联动。** `Queued；原准权威 Q-03；执行首步冻结归属规则`
