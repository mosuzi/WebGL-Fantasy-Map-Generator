...
> **执行门禁（2026-07-29）**：第 45～52、54～212 项已完成，第 53 项已移除；上述完成项不得重新执行各自原验收或重新计入待办。用户已整体批准 `Q-28～Q-33`，按依赖顺序登记为第 211～216 项；当前只执行第 213 项，其余按封闭顺序等待。第 216 项增加用户明确要求：管理 Tab 下编辑弹框的高度预算必须计入控制面板入口实际占用的一行。
...
- **权威任务第 213 项：统一动作坞语义、ARIA 与双击选择。** `活动；来源：Q-30，用户明确批准`
...
- **权威任务第 214 项：补齐定位、重命名 no-op 与画布模式反馈。** `排队；来源：Q-31，用户明确批准`
