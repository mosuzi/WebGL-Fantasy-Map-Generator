...
269. 地区纹理图层第一刀已完成：复用已有 `zones` 数据作为“地区”图层，控制面板新增“地区”开关，renderer 新增 WebGL 纹理绘制层，支持斜线、交叉线和圆点阵列三类基础纹理；国家和省份默认颜色仍保持纯色，不参与纹理。生成端降低默认地区密度到 `2-6` 个，新增 `Warzone` 类型，优先从实际军事 front / Enemy 边界附近生成少量战区；已有大型宗教战区范围被压缩，避免满图纹理。地区导出补充 `pattern / hexColor`。后续待做：地区管理面板、手动创建地区、颜色选择器中的纹理选择入口。
