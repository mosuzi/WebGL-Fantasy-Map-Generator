...
- 战斗事件链路列表与 CSV 导出已补可读元信息：选中军团事件列表的每条事件现在显示链路序号、已应用/未应用状态和损耗状态，CSV 事件导出也会带出同样的链路、应用状态和损耗状态字段；事件超过 5 条时可展开完整筛选链路，默认仍只渲染最近 5 条。本轮不改变事件保存、导入、清理或轻量结果应用。
