...
280. 地区纹理可见性已修正：地图地区层不再主要依赖实心半透明填色，zone cell 底色压到很弱的辅助提示；斜线、交叉线和圆点阵列改为每个 cell 内多线 / 多点绘制，并把大地区的纹理抽样上限从 `220` 提到 `720`，避免远看像实心色块或纹理过疏。地区管理面板图例的斜线、交叉线和圆点也同步加密，与地图表现一致。构建通过，并用构建产物截图确认入侵区在地图上显示为斜线纹理而不是实心高亮。
