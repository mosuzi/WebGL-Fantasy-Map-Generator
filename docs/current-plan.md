...
- **权威任务第 335 项：GPU 常驻视图与零重编译切换。** `进行中；来源：用户要求根治 100k 视图切换端到端卡顿，不能再用 Worker 算法耗时冒充上屏性能`
  ...
  - **阶段状态**：335-A、335-B 已接受并分别以 `0.3.17 / 0.3.18` 建立本地 checkpoint。固定 100k 暖普通颜色为 `101～186ms / 3` 输入包；revision 推进后的 cache 失效为 `5.02s`，session 丢失 cold 为 `21.07s / 1032` 包。B 已把正式 surface base 从每顶点 `x/y/rgba/side` 改为 `x/y/packed(cellId, side)` 的稳定 12-byte 几何；颜色在 335-C 接管前使用独立 12-byte Float32 RGB 兼容流，局部颜色补丁只写颜色 buffer，不再重传位置或 identity。10k 正式 `452091` 顶点的几何 / 颜色各 `5425092B`，packed 与 legacy base 全 framebuffer 逐通道差异为 `0`，事务回滚、14 类视图、LongTask、应用错误、Loading 与 WebGL error 全部通过。当前唯一施工阶段为 335-C。
