...
12. 军事单位编辑面板已完成概要、比例面板、态势筛选和手动态势调整：当前“军事管理”已新增选中军团概要、状态标签、重点指标、兵种构成条、兵种比例可视项、态势下拉筛选，并支持通过二级操作可撤销地修改单个军团态势；后续军事方向应转向批量命令、军团重命名/驻地编辑、战斗事件和更完整的战报链路，避免继续只做展示层微调。
