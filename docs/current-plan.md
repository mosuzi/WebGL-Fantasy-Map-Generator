...
2026-07-16 国家生命周期第 61～63 项已经完成并通过专项回归、生产构建和同一 Chrome 内的集中交互验收，现已移出活动清单。重新审计 `FOLLOWUPS.md`、专题计划、代码接线和现有回归后，先确认第 64～65 项两个维护问题；随后根据用户在适配视图下的视觉反馈，补充第 66 项道路与河流尺度宽度整改。
...
> **执行门禁（2026-07-16）**：第 45～52、54～63 项已完成；第 53 项已移除。当前只执行第 64～66 项，顺序固定为 `64 -> 65 -> 66`，前一项未验收前不得开始后一项。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、186 个公开方法和 90 个编辑方法，稳定等级为 `178 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。

继续采用目标模式：只实现第 64～66 项的封闭范围，达到当前项最小验收后立即转入下一项；不影响当前验收的新发现只写入 `FOLLOWUPS.md`。第 64～65 项以代码检查和专项回归快速迭代，不要求逐项启动浏览器；第 66 项属于视觉渲染整改，在代码门禁通过后集中执行一次 Chrome 视觉验收。
...
除第 64～66 项外，当前没有其它活动权威任务。纹章深化、动态军事、3D / OBJ、子地图变换、富文本备注等仍为明确暂缓或远期候选，不作为隐含待办。
