...
第 87 项已采用“调查 → 开发 → 代码审查”流程并单独提交，代码完成后只执行本项浏览器验证；第 64～86 项没有复测。不影响本项验收的新发现没有扩展实施。当前权威任务清单为空。

2026-07-18 用户整体批准下一批准权威候选 `Q-12～Q-17`，现按原顺序转为权威任务第 88～93 项。每项使用“调查 → 开发 → 审查测试”三步完成并单独提交，中途不做浏览器验证；六项代码全部完成后才复用同一浏览器统一验收，已完成第 45～52、54～87 项不得复测。全部验收通过后统一推送。
...
> **执行门禁（2026-07-18）**：第 45～52、54～87 项已完成，第 53 项已移除；上述完成项不得复测或重新计入待办。当前活动任务为第 88～93 项，必须严格连续执行，前一项完成调查、开发、审查测试并提交后才能开始后一项；全部完成后统一进行浏览器验证并推送。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、204 个公开方法和 106 个编辑方法，稳定等级为 `196 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。
...
- **权威任务第 88 项：修正高度普通笔刷的抬手提交与取消回滚。** `代码完成；浏览器留到第 88～93 项统一验收；来源：Q-12 / FOLLOWUPS`
...
- **权威任务第 90 项：治理气候下游重算的主线程长任务。** `代码完成；真实性能留到第 88～93 项统一浏览器验收；来源：Q-14 / 第 72 项性能记录`
  ...
  - 实现与审查记录：新增气候下游异步分段事务，在重算前快照、各系统、重算后快照和历史登记之间统一让出主线程并报告 loading 阶段；气候专用 marker / economy 组合步骤跳过内层 renderer / panel 刷新，成功后只统一刷新一次。面板增加运行态、重复提交保护和“重算中”反馈，公开 API 元数据同步声明 async；同步事务仍保留供既有内部调用，异步结果、失败回滚、单历史和存档语义均有等价回归。固定 10k Node 样本四段同步块为约 `128.59 / 159.18 / 116.35 / 0.13ms`，最大 `159.18ms < 250ms`，总耗时约 `406.18ms`；气候专项、市场归属、API stability / capabilities / operation、语法、生产构建和差异检查通过。真实 Chrome health 证据留到六项统一阶段。

- **权威任务第 91 项：商品与市场展示属性轻量编辑。** `代码完成；浏览器交互留到第 88～93 项统一验收；来源：Q-15 / 经济计划阶段 3`
  ...
  - 实现与审查记录：新地图为商品写入确定性的可见状态、颜色、图标和标签，为市场写入确定性颜色；完整地图导入 / 导出统一归一化旧图缺字段。经济面板在商品和市场详情中提供一次保存的展示编辑表单，列表同步显示商品图标、标签、显隐状态和颜色色块以及市场名称、颜色色块；`edit.economy.setGoodDisplay / setMarketDisplay` 与面板共用同一 `EditHistory` 命令并同步 `pack / economy` 镜像。专项回归确认每次保存只增加一条历史、撤销 / 重做和新旧地图往返一致、约 1.4MB 的经济数值 checksum 不变；市场归属、API inventory / stability / edit coverage / action convergence / data compatibility、语法、生产构建和差异检查通过。公开基线更新为 `206 / 108`，真实面板交互留到六项统一浏览器阶段。

- **权威任务第 92 项：贸易数据结构化组合查询与关联定位。** `代码完成；浏览器交互留到第 88～93 项统一验收；来源：Q-16 / 经济计划阶段 4`
