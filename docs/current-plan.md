...
- 新增对象 affected helper 第一刀已完成：`edit-command-effects.js` 新增 `newObjectAffected(kind)`；新增城市、省份、国家、空文化、空宗教、手工标签、marker、保存测量对象和测量对象导入命令的初始 `kind#new` 已改为复用该 helper，执行成功后的真实 id 回写保持不变；`edit-command-contract.md` 和编辑器基础设施清单已同步 helper 使用约定。
