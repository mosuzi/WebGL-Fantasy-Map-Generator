...
> **执行门禁（2026-08-20）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-10g-a` 已由独立只读评审 `ACCEPT`。当前进入最终 `349-11`：执行全量非浏览器终验，形成并评估浏览器验收方案与防误触门，但不启动、不操作、不执行任何浏览器门。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`。
...
- 状态：进行中，`349-0`～`349-10g-a / 0.5.52` 已接受；当前执行 `349-11` 非浏览器终验与浏览器验收方案评估。
