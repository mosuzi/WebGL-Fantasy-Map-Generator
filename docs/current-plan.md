...
> **执行门禁（2026-08-20）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-10g` 已由独立只读评审 `ACCEPT`。当前执行动态插入的 `349-10g-a`：只同步 349-7 markers 专项与 349-10d 已接入的正式 Worker / regeneration 契约，不回退产品 Manifest。完成并独立接受后才进入 `349-11`。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，最终阶段只形成并评估浏览器验收方案与防误触门。
...
- 状态：进行中，`349-0`～`349-10g / 0.5.50` 已接受；当前执行动态插入的 `349-10g-a`，随后进入 `349-11`。
