...
- `UiObjectTable.columnWidths` 覆盖入口已完成：公共表格新增可选 `columnWidths` prop，可按列 key 覆盖列定义里的 `width` 并继续复用 `minWidth / maxWidth` 归一化逻辑；未传该 prop 的现有面板行为保持不变。本步只补组件入口，不接拖拽改宽 UI、不改变任何面板默认列宽。`git diff --check` 和 `.\node_modules\.bin\vite.cmd build --config vite.config.mjs` 通过，构建仅有既有 Vite 大 chunk 警告；浏览器烟测待本批次收尾统一执行。
