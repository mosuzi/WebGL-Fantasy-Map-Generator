...
> **执行门禁（2026-08-20）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-10e` 已由同一只读评审智能体 `ACCEPT`，当前进入 349-10f 的 generation / import / adoption / export / headless profile 收口；先冻结真实 owner、事务和专项门，不删除未证明冗余的 legacy adapter。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，349-7 与 349-10c 各误触一次命名不显式的浏览器脚本，均已立即停止且不计验收，最终阶段只形成并评估浏览器验收方案与防误触门。
...
- 状态：进行中，`349-10e / 0.5.44` 已接受，`349-10f / 0.5.45` 已开启；`349-0`～`349-10e` 已接受。
