...
- **权威任务第 233 项：重写人类友好的中英文 README。** `已完成；来源：用户直接要求`
  ...
  - 自检记录：两版相对链接逐项解析均存在，语言切换互链正确；Node.js、pnpm、`dev / build` 命令和默认地址已与当前配置核对。线上入口与原项目致谢链接均返回 HTTP `200`，线上页面标题为“莫苏子的架空地图生成器”；`audit:wiki`、`audit:ai-docs` 与 `git diff --check` 通过。
