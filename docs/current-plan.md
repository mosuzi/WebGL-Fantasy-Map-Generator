...
12. 军事单位编辑面板需要进入样式与交互整改：当前“军事管理”功能已经有筛选、定位、导出和比例调整，但编辑面板视觉密度、二级操作样式和对象详情层级仍明显粗糙，后续应按现有 `UiObjectTable / UiActionDock / UiColorActionPanel` 风格重做，而不是继续堆临时字段。
13. 国名随机需要补方位语义约束：国家根名可以继续使用东/西/南/北等变体，但带方位前缀时应结合国家中心相对同名文化/同根国家的位置判断；北边国家不应随机叫“南某”，南边国家不应随机叫“北某”，除非其对应方向上确实存在同名或同根参照国。
14. README 需要在本轮收束后刷新：把已经完成的生成、编辑、导入导出、贸易外交军事、性能与 WebGL 化进展写成读者能看懂的“已完成”，并把军事面板、国名方位、README 自身之外的剩余任务整理成“还要做”。
