...
2026-07-18 第 64～81 项全部完成并统一验收、推送。用户随后明确批准五项新的界面与生成体验整改，按原始顺序登记为第 82～86 项；本轮只按这五项连续执行，不从历史缺口或候选文档追加其它任务。

本轮继续采用快速迭代：日常实现以代码检查和专项回归为主，第 82～86 项代码全部完成后再集中执行一次浏览器验证。每项达到非浏览器最小验收并提交后立即进入下一项，不影响当前验收的新发现只记录到 `FOLLOWUPS.md`。
...
> **执行门禁（2026-07-18）**：第 45～52、54～81 项已完成，第 53 项已移除；上述完成项不得复测或重新计入待办。当前只执行第 82～86 项，顺序固定为 `82 -> 83 -> 84 -> 85 -> 86`，前一项完成“调查 → 开发 → 代码审查”和本地提交后再进入下一项。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、204 个公开方法和 106 个编辑方法，稳定等级为 `196 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。

继续采用目标模式：只实现第 82～86 项的封闭范围，不影响当前验收的新发现只写入 `FOLLOWUPS.md`。每项严格按“调查 → 开发 → 代码审查”实施并单独提交，五项代码全部完成后再统一执行第 82～86 项浏览器验证；第 64～81 项已经完成，不再复测。统一验收通过后再一次性推送本轮全部提交，不在中途逐项推送。
...
- **权威任务第 82 项：未打开过的管理面板首次默认位于画布左侧。** `代码与自动化完成，待第 82～86 项统一浏览器验证；来源：用户 2026-07-18 反馈`
...
- **权威任务第 83 项：提高自动生成湖泊名称使用“湖”的比例。** `代码与自动化完成，待第 82～86 项统一浏览器验证；来源：用户 2026-07-18 反馈`
...
- **权威任务第 84 项：禁止选择画布及地图叠加层文本。** `代码与自动化完成，待第 82～86 项统一浏览器验证；来源：用户 2026-07-18 反馈`
...
- **权威任务第 85 项：统一主题导入与导出按钮文字规格。** `代码与自动化完成，待第 82～86 项统一浏览器验证；来源：用户 2026-07-18 反馈`
...
- **权威任务第 86 项：收紧水体与地貌统计详情区排版。** `代码与自动化完成，待第 82～86 项统一浏览器验证；来源：用户 2026-07-18 图 1 反馈`
...
第 64～81 项已全部完成；当前活动权威任务只有第 82～86 项。纹章深化、动态军事、贸易动画、3D / OBJ、子地图变换、真实 CRS、富文本备注和新导出格式等仍为明确暂缓或远期候选，不作为隐含待办。
