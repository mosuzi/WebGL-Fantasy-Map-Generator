...
2026-07-16 国家生命周期第 61～63 项已经完成并通过专项回归、生产构建和同一 Chrome 内的集中交互验收，现已移出活动清单。重新审计 `FOLLOWUPS.md`、专题计划、代码接线和现有回归后，先确认第 64～65 项两个维护问题；随后根据用户在适配视图下的视觉反馈补充第 66 项道路与河流尺度宽度整改，根据所有编辑面板反复强制停靠右侧的实测反馈补充第 67 项手动位置优先整改，根据画笔范围过大且不可见的实测反馈补充第 68 项笔刷大小与光标范围预览整改，并继续补充第 69 项标签类型样式系统和第 70 项夸克网盘保存集成调研。
...
> **执行门禁（2026-07-16）**：第 45～52、54～63 项已完成；第 53 项已移除。当前只执行第 64～70 项，顺序固定为 `64 -> 65 -> 66 -> 67 -> 68 -> 69 -> 70`，前一项未验收前不得开始后一项。

当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、186 个公开方法和 90 个编辑方法，稳定等级为 `178 / 7 / 1`；第 28～52 项均已达到各自封闭范围内的最小验收并移出活动清单。

继续采用目标模式：只实现第 64～70 项的封闭范围，达到当前项最小验收后立即转入下一项；不影响当前验收的新发现只写入 `FOLLOWUPS.md`。第 64～65 项以代码检查和专项回归快速迭代，不要求逐项启动浏览器；第 66～69 项属于视觉或真实交互整改，均在代码门禁通过后各集中执行一次 Chrome 验收；第 70 项只产出可追溯的可行性结论和后续实施建议，不得在调研项内擅自接入非官方接口。
...
除第 64～70 项外，当前没有其它活动权威任务。纹章深化、动态军事、3D / OBJ、子地图变换、富文本备注等仍为明确暂缓或远期候选，不作为隐含待办。
