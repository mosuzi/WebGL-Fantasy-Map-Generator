...
- **权威任务第 76 项：文化与宗教中心、扩张参数编辑。** `代码与自动化完成，待第 76～81 项统一浏览器验收；原准权威 Q-06`
...
- **权威任务第 77 项：数值适居度笔刷。** `代码与自动化完成，待统一浏览器验证；原准权威 Q-07`
...
- **权威任务第 78 项：人口手工调整第一刀。** `代码与自动化完成，待统一浏览器验证；原准权威 Q-08`
...
- **权威任务第 79 项：补齐 Source 地形模板操作。** `代码与自动化完成，待统一浏览器验证；原准权威 Q-09`
...
- **权威任务第 80 项：标签显示优先级与位置锁定。** `代码与自动化完成，待统一浏览器验证；原准权威 Q-10；依赖第 69 项标签类型 registry`
...
- **权威任务第 81 项：补齐路线与河流 GeoJSON 稳定属性。** `代码与自动化完成，待统一浏览器与外部读取验证；原准权威 Q-11`
  ...
  - 审查记录：稳定字段只使用现有对象、点列和已存水文值，不推导交通量、通航、流速、坡降或真实单位；未知代码、空关系和缺省统计均有固定规则。专项覆盖固定生成图、全图 / 等价 bbox 完全一致、层内字段集合与类型、旧键、未知代码、JSON 往返及旧消费者；范围、路线、河流、对象创建、七步导出套件、生产构建与 `git diff --check` 已通过。QGIS 专项已补网络线层和字段断言，但旧便携 QGIS 目录本轮已不存在，不能把本轮新增字段写成 QGIS 实读通过；geojson.io 与浏览器证据留到第 76～81 项统一验证。
