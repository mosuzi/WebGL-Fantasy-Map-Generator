...
1. **面板布局深场景审计补齐**：
   - 自动审计工具已经覆盖 `1280 x 820` 和 `1024 x 720` 下的国家、省份、城市、文化、宗教、外交、政体、经济、军事、路线、河流、湖泊、资源标记、测量对象和名称库等主要面板。
   - 下一步补长字段选中样本、表格列较多样本和二级编辑区展开态，而不是重复默认 / 窄视口样本。
   - 审计项继续聚焦 body 横向滚动、summary/detail 最小项宽、segmented / sort bar 换行、按钮文字溢出、表格列压缩和长字段异常折行。
2. **面板空间策略修正**：
   - 只修审计或本地预览能复现的具体折行、错位和窄控件，不做单个面板的大重排。
