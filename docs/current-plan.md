...
> **执行门禁（2026-08-19）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-9` 已由同一只读评审智能体 `ACCEPT`，`349-10a` terrain / grid / height-derived / climate / ocean / topology 基础域已形成 `0.5.26` checkpoint 候选，正在等待同一智能体评审，未获 `ACCEPT` 不进入 `349-10b`。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，349-7 曾误触发一次带浏览器的既有导出脚本，已首败停止且不计为验收，只在最终阶段形成并评估浏览器验收方案。
...
- 状态：进行中，`349-10a` checkpoint 待评审；`349-0`～`349-9` 已接受。
