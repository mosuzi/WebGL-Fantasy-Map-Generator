...
当前首先全面完成控制台 / 扩展 API 系统。权威任务第 28～30 项已经完成能力盘点、范围冻结、既有编辑命令覆盖和 UI / API 公共 action 收口，剩余第 31～34 项继续实施“统一异步事务、旧数据完整证明、稳定版本契约和一键聚合门禁”；API 完成后进入依赖稳定 API 的第 35 项快捷键机制，再按第 36～44 项执行由 `FOLLOWUPS.md` 提升的编辑器、创作、导入诊断和导出增强。

下一波继续采用快速迭代：第 31～33 项以代码检查和专项回归为主，不逐项启动浏览器；第 34 项集中执行一次真实页面综合验收。每项达到最小验收后立即进入下一项，不影响当前验收的新发现只记录到 `FOLLOWUPS.md`。
...
下一波先全面完成“控制台 / 扩展 API 系统”，再执行依赖稳定 API 的快捷键机制，随后推进从 FOLLOWUPS 提升的九项任务。当前 API 基线是：`window.webglGeneratorApi` 已覆盖 11 个命名空间、158 个公开方法和 72 个编辑方法；权威任务第 28 项完成 34 类能力盘点与三方覆盖自检，第 29 项把其中 20 个可纯参数调用的既有编辑命令全部接入 API，第 30 项建立应用级唯一 `runtimeActions`，让生成 / 重算、图层 / 显示、气候、导入导出、选择和名称库等 UI / API 入口共用 action，并补齐 4 个显示偏好和 2 个高度派生方法。尚未完成之处是：异步长任务状态未统一，所有接口仍是 `experimental / draft`，也没有一条覆盖整套 API 的聚合门禁。第 35 项不得在 API 完成前提前施工；第 36～44 项已从 FOLLOWUPS 正式移入本清单，不再作为阶段外备忘。

权威任务第 28～30 项已达到最小验收并移出活动清单；完成证据见 `docs/task-notes/console-api-capability-inventory.md`、`regress:api-inventory`、`regress:api-edit-coverage`、`regress:api-action-convergence` 与 `docs/development-log.md`。

1. 异步长任务与错误事务统一。`权威任务第 31 项，当前执行项`
...
2. API 数据兼容与完整往返补齐。`权威任务第 32 项，待第 31 项后执行`
...
3. API 稳定性、版本与扩展契约。`权威任务第 33 项，待第 31～32 项后执行`
...
4. API 聚合门禁与阶段末综合验收。`权威任务第 34 项，最后执行`
...
5. 常用操作键盘快捷键与悬停提示。`权威任务第 35 项，依赖第 28～34 项；API 未完成时跳过`
...
6. 统一画布工具模式管理器。`权威任务第 36 项，来自 FOLLOWUPS`
...
7. Selection handler 自动打开面板策略统一。`权威任务第 37 项，来自 FOLLOWUPS；需要产品决策`
...
8. 选区地形模板完整体系。`权威任务第 38 项，来自 FOLLOWUPS`
...
9. 破坏性操作确认、依赖预览与批量删除。`权威任务第 39 项，来自 FOLLOWUPS`
...
10. 路线绘制、河流新增与湖泊开挖。`权威任务第 40 项，来自 FOLLOWUPS`
...
11. Zone、通用标记与独立备注创建 / 删除闭环。`权威任务第 41 项，来自 FOLLOWUPS`
...
12. GEO、Cells GEO、高度图统一诊断与 schema 演进。`权威任务第 42 项，来自 FOLLOWUPS`
...
13. 政治面外部 GIS 互操作与 100k 浏览器导出验证。`权威任务第 43 项，来自 FOLLOWUPS`
...
14. PNG 任意裁剪与细粒度 overlay 选择。`权威任务第 44 项，来自 FOLLOWUPS`
