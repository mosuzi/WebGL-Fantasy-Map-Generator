...
> **执行门禁（2026-08-19）**：第 349 项“地图核心引擎化与 TypeScript 核心契约渐进迁移”已获用户明确批准；`349-0`～`349-8` 已由同一只读评审智能体 `ACCEPT`，当前进入 `349-9` dependency registry、projection 状态与局部失效接线。该任务仅在 `codex/map-core-engine-architecture-plan` 并行分支推进，不得合入 `main`；禁止浏览器验收，349-7 曾误触发一次带浏览器的既有导出脚本，已首败停止且不计为验收，只在最终阶段形成并评估浏览器验收方案。
...
- 状态：进行中，当前阶段 `349-9`；`349-0`～`349-8` 已接受。
