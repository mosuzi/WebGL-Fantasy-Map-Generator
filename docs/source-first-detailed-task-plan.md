...
本文档是 `docs/source-first-recovery-execution-plan.md` 的细化施工图。旧文档负责说明为什么复位和大阶段顺序；本文档负责让后续太子、尚书、门下、侍中可以直接按阶段开工、复核和验收。
...
`quick` 模式覆盖 `mediterranean`、`continents`、`archipelago` 的 100000 cells 强回归样例；`full` 模式覆盖 7 个模板、3 档 cells、每模板 3 个 seed。工具会生成 `docs/source-baselines/matrix.json` 和 `docs/source-baselines/matrix.md`。
...
```text
docs/source-baselines/<case>/source-summary.json
docs/source-baselines/<case>/candidate-summary.json
docs/source-baselines/<case>/diff.json
docs/source-baselines/<case>/*.png（本地预览产物，默认不纳入版本库）
docs/source-baselines/<case>/validation.md
```
