...
本文档记录对照原版 `Namesbase Editor` 后，WebGL 版名称库能力的实现边界。当前生成器已有中文词池、春秋古国风国家根名和标签管理，也已具备用户名称库导入导出、编辑、全局绑定、文化级绑定、文化面板快捷入口、应用级本地偏好和样本权重；后续重点是链路质量和更接近原版的 Markov 生成。
