...
本文档记录对照原版 `Namesbase Editor` 后，WebGL 版名称库能力的实现边界。当前生成器已有中文词池、春秋古国风国家根名和标签管理，也已具备用户名称库导入导出、编辑、全局绑定、文化级绑定、文化面板快捷入口和应用级本地偏好；后续重点是权重、链路质量和更接近原版的 Markov 生成。
...
1. 让全局 `stateRoot / place / hydro` 绑定进入后续显式重新生成，但不自动改已有名称。
2. 再考虑文化级绑定和显式重命名命令。
3. 需要更接近原版时，再追加纯函数 Markov chain 与当前轻量预览对照。
