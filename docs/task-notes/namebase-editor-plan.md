...
本文档记录对照原版 `Namesbase Editor` 后，WebGL 版名称库能力的实现边界。当前生成器已有中文词池、春秋古国风国家根名和标签管理，也已具备用户名称库导入导出、编辑、全局绑定、文化级绑定、文化面板快捷入口、应用级本地偏好、样本权重和项目内 Markov 链路生成；后续重点是独立编辑历史栈和显式重命名命令。
...
2. 再考虑文化级绑定和显式重命名命令。
3. 需要更接近原版时，再补 `min / max / duplicateChars` 等更完整参数。
