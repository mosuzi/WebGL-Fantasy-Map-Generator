...
- 新协议正例覆盖四个 Manifest，写路径分别为 features `25`、routes `21`、rivers `31`、markers `28`，route-path 为两类真实 history roots 的 `21` 路径并集；两轮评审后补齐普通 Feature 引用 / 镜像与 source identity、route cell links、river topology / 逐段 owner，拒绝集共 `23` 类，并以 3 个额外 10k 河网种子覆盖合法河口与父子汇流例外。
...
两轮评审共七项 P1 已在 `0.5.38` blocker-only 候选中按原边界闭合；没有新增阶段，未完成顺序不变。
