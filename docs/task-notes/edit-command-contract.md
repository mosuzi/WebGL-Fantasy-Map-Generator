...
- 新增城市、省份、国家、空文化、空宗教、手工标签、marker、保存测量对象和测量对象导入命令的初始 affected 已改为复用 `newObjectAffected(kind)` 生成 `kind#new`，执行成功后仍会替换为真实对象 id；marker 视觉、备注、移动、删除和资源点重生成命令，标签移动、重命名、备注、删除、恢复命令，测量对象保存回写、重命名、更新、删除和导入回写命令，文化 / 宗教新增回写、删除、颜色、继承父级命令，以及路线、河流、地区样式和备注删除的单对象命令已开始复用 `objectAffected(kind, id)` 生成单对象目标。
