...
- 第一批低风险命令已主动声明 `domain`：路线、河流、湖泊、地区、备注和测量对象。
- 面板历史按钮已开始复用 `executeHistoryCommand()`，避免各面板分别手写撤销 / 重做刷新。
- 后续重点是继续迁移残留的直接 `state.editHistory.execute()` 路径，并让城市、国家、省份、文化、宗教、marker、标签、外交、军事等命令继续补齐 `domain` 和更精确的 `effects.affected`。
