...
- 新增城市、省份、国家、空文化、空宗教、手工标签、marker、保存测量对象和测量对象导入命令的初始 affected 已改为复用 `newObjectAffected(kind)` 生成 `kind#new`，执行成功后仍会替换为真实对象 id；marker 视觉、备注、移动、删除和资源点重生成命令已开始复用 `objectAffected(kind, id)` 生成单对象目标。
