...
- 国家、省份、道路、河流、城市重生成入口、资源点重生成命令、军事批量 / 事件命令、城市 / 国家 / 河流 / 湖泊按名称库批量重命名命令，以及高度 / 国家 / 省份刷子和 FMG Cells GEO 导入命令，已开始使用 `derived-system#xxx` 作为批量 affected 的第一项，避免历史和刷新摘要只出现对象级 `all`、集合别名、一串同类对象 id 或 `grid-cells#数量`。
