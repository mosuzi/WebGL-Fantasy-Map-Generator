...
> 状态校准：这是 2026-06-18 的历史复位总纲，对应阶段已经执行并形成后续正式应用能力。文中的“当前下一步”和冻结要求不再代表活动任务；当前批准范围只看 [`../current-plan.md`](../current-plan.md#权威任务清单)。
...
本文档是复位总纲。后续执行以更细的施工图 `docs/task-notes/source-first-detailed-task-plan.md` 为准。该详细规程补充了独立新智能体 source 复查后的缺口清单、完整生成顺序、字段契约、source baseline 导出 schema、模板/seed/cells 矩阵、各阶段任务包和可脚本判定的验收要求。

在 `docs/task-notes/source-first-detailed-task-plan.md` 的阶段 0 完成前，不应继续修改正式应用生成器。
