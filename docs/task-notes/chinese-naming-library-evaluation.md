...
> 状态校准（2026-07-16）：本文是阶段 18 的历史调研。名称库管理、绑定、Markov 生成、质量参数和多词率 `m` 已在后续任务中落地；剩余建议不自动成为当前待办。
...
第 83 项补充：新地图批量湖泊命名会显式采用 `湖 7 / 其它合法后缀 4` 的常用类型词权重，使“湖”在足量样本中形成严格多数；后缀字典本身、默认名称生成器、手工建湖和显式重命名仍保持原契约，不自动改写旧地图名称。

第 115 项补充：中国风省份不再按全局 province ID 轮换“领”等后缀，而是由所属国家稳定选择州、郡、府、道、路、军、镇、司之一，同国普通省份统一；无 burg 的未归属块仍可明确命名为“边地”。旧图已有完整省名不自动改写，只补缺失后缀。

第 116 项补充：运行时新增集中式文化命名风格 registry，东方、欧陆、英伦、古典、高幻想与暗黑幻想均显式登记文化根、地点根、水系词根和形制；八种文化集合不再靠空 `nameStyle` 隐式回退。欧陆与英伦各保有 `32` 个独立地点根名，名称生成器按对象所属文化生成城市、国家和省份词根；`world / random` 才允许混合。该 registry 只约束新生成结果，不迁移旧对象名称，也不替换用户名称库。
