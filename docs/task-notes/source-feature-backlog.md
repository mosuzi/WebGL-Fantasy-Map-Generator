...
| 名称库编辑器 | `source/Fantasy-Map-Generator/src/controllers/namesbase-editor.ts` | 当前有中文命名策略、标签管理、只读名称库总览和内置名称库 JSON 导出；用户仍不能导入、维护名称库、调整权重或做文化绑定；实现计划已落到 `docs/task-notes/namebase-editor-plan.md`。 | 中等 | 下一步先做用户自定义名称库的导入/编辑策略，再做生成绑定。 |
...
3. 名称库编辑器：计划、只读总览和内置名称库导出已完成；下一步先做用户自定义名称库的导入/编辑策略，避免直接改生成器导致命名回退。
