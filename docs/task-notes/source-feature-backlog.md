...
|---|---|---|---|---|
| 测量工具：直尺、曲线尺、路线尺、面积尺 | `source/Fantasy-Map-Generator/public/modules/ui/measurers.js`，`Ruler / Opisometer / RouteOpisometer / Planimeter` | 已完成临时折线测距和闭合多边形面积第一刀；尚未支持路线贴合、保存测量对象和编辑节点。 | 中等 | 下一步补节点拖拽、路线贴合和保存/导出测量结果。 |
...
1. 测量工具：折线测距和面积第一刀已完成；后续可做节点拖拽、路线贴合、保存与导出。
