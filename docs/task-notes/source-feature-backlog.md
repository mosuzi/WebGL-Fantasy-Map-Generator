...
| 对象注记 | `source/Fantasy-Map-Generator/public/modules/ui/notes-editor.js`，`editNotes` | 数据契约和分阶段入口已落到 `docs/task-notes/object-notes-implementation-plan.md`；marker、city、river、route、state、province、culture、religion 与 label 纯文本备注第一刀已完成，尚未做独立总览、孤儿备注标记和富文本。 | 中等 | 下一步补独立备注总览；富文本编辑器和 AI 辅助暂缓。 |
...
2. 对象注记：为独立备注总览、孤儿备注标记和战斗事件后续铺路。
