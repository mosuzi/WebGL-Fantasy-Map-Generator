...
| 对象注记 | `source/Fantasy-Map-Generator/public/modules/ui/notes-editor.js`，`editNotes` | 数据契约和分阶段入口已落到 `docs/task-notes/object-notes-implementation-plan.md`；marker、city、river、route、state、province、culture、religion 与 label 纯文本备注第一刀已完成，独立备注总览第一刀已完成，尚未做富文本、独立导入导出和孤儿备注批量清理。 | 中等 | 下一步可补备注独立导入导出、孤儿备注批量操作；富文本编辑器和 AI 辅助暂缓。 |
...
| 分层 GeoJSON 导出 | `source/Fantasy-Map-Generator/public/modules/io/export.js`，`saveGeoJsonRoutes / Rivers / Markers / Zones` | 已完成城市、路线、河流、marker 和 zone 要素 GeoJSON 第一刀；尚缺国家/省份/区域 dissolve、范围选择、分层选择和更完整属性映射。 | 中等 | 下一步补国家/省份 dissolve 或分层选择；SVG、瓦片 zip 暂缓。 |
...
2. 对象注记：第一批专用入口和独立总览已完成；后续为孤儿备注批量操作、独立导入导出和战斗事件备注铺路。
