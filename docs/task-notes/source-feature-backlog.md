...
| 对象注记 | `source/Fantasy-Map-Generator/public/modules/ui/notes-editor.js`，`editNotes` | 数据契约和分阶段入口已落到 `docs/task-notes/object-notes-implementation-plan.md`；marker 纯文本备注第一刀已完成，尚未接入城市、河流、路线、国家、省份、文化、宗教、标签和独立总览。 | 中等 | 下一步补 city / river / route 入口；富文本编辑器和 AI 辅助暂缓。 |
