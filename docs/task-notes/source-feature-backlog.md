...
|---|---|---|---|---|
| 测量工具：直尺、曲线尺、路线尺、面积尺 | `source/Fantasy-Map-Generator/public/modules/ui/measurers.js`，`Ruler / Opisometer / RouteOpisometer / Planimeter` | 已完成临时折线测距、闭合多边形面积、测量 JSON 导出、节点拖拽、撤销最后一点、节点删除、线段插入节点、保存临时测量为对象和保存对象图层显隐第一刀；测量对象与路线贴合计划见 `docs/task-notes/measurement-rulers-plan.md`；尚未支持路线贴合和已保存对象节点编辑。 | 中等 | 下一步按专项计划做已保存对象节点编辑，再做路线贴合。 |
...
1. 测量工具：折线测距、面积、测量 JSON 导出、节点拖拽、撤销最后一点、节点删除、线段插入节点、保存临时测量为对象和保存对象图层显隐第一刀已完成；测量对象与路线贴合计划已落到 `docs/task-notes/measurement-rulers-plan.md`，后续先做已保存对象节点编辑，再做路线贴合。
