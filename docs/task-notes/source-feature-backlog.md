...
|---|---|---|---|---|
| 测量工具：直尺、曲线尺、路线尺、面积尺 | `source/Fantasy-Map-Generator/public/modules/ui/measurers.js`，`Ruler / Opisometer / RouteOpisometer / Planimeter` | 已完成临时折线测距、闭合多边形面积、测量 JSON 导出、节点拖拽、撤销最后一点、节点删除和线段插入节点第一刀；尚未支持路线贴合和保存测量对象。 | 中等 | 下一步补路线贴合和保存测量对象。 |
...
1. 测量工具：折线测距、面积、测量 JSON 导出、节点拖拽、撤销最后一点、节点删除和线段插入节点第一刀已完成；后续可做路线贴合和保存测量对象。
