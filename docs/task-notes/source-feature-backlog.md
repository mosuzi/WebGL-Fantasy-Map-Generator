...
| 名称库编辑器 | `source/Fantasy-Map-Generator/src/controllers/namesbase-editor.ts` | 当前有中文命名策略、标签管理、只读名称库总览、当前名称库 JSON 导出、名称库 JSON 导入保存、单个删除用户库和清空用户库；用户仍不能编辑词池、调整权重或做文化绑定；实现计划已落到 `docs/task-notes/namebase-editor-plan.md`。 | 中等 | 下一步先做用户库编辑、复制内置库和导入覆盖/追加策略，再做生成绑定。 |
