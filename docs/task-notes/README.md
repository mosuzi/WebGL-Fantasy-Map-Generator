...
- 第 371 项以 `1af5381 / v0.5.4` 需求前基线复现存档载入国界颜色过渡，再只在共享边界实验室以合法三方夹具比较旧颜色带、现九轨与连续朦胧候选；用户确认前不改正式 renderer；见 [`task-371-state-border-blend-lab.md`](./task-371-state-border-blend-lab.md)。
