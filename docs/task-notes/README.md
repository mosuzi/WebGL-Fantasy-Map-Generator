...
- [第 401 项列表身份显示与选择交互](./task-401-player-list-identifiers.md)：隐藏 ID、Ctrl 多选、右侧固定列与城市锁状态同步；709 状态验收及首败收敛证据。
- [第 400 项界面排版规则与自动验收](./task-400-ui-layout-contract.md)：普通按钮单行、显式多行例外、状态覆盖、构建 / 提交凭据和同类修复证据。
- [第 386 项回归校准](./task-386-regression-calibration.md)：源码归因、断言改动与待统一功能验收的道路连通分类；只通过静态检查。
- [地图内容搜索与编辑面板精简（2026-09-19）](./map-search-and-panel-refinement-2026-09-19.md)：第 391 项扩展搜索、第 396 项面板图标 / 分组 / 布局 / 去冗余 / 明亮主题的源码依据及阶段验收；制造约束和动态结算恢复候选的边界。
- [项目深化评估（2026-09-18）](./project-deepening-assessment-2026-09-18.md)：第 386～394 项原设计依据、回归证据、第 395 项赔款移除边界及交互协同评估；9 月 19 日搜索扩围与 D-02 / D-03 恢复按最新说明。执行状态只见 `current-plan.md`，未批准建议只见根 `FOLLOWUPS.md`。
...
- 第 367 项把 GPU 紧凑 surface 的 cell identity / ranges 缺失从“直接拒绝”改为“从 canonical grid 兼容补建，补建后再校验”，并保留基础网格不可恢复时的结构化失败；见 [`task-367-render-coverage-compatibility.md`](./task-367-render-coverage-compatibility.md)。
...
- 第 349-7 阶段的 marker presentation 共享源、detached query、point layer / picking / DTO / Feature GeoJSON 同源契约见 [`task-349-markers-presentation-slice.md`](./task-349-markers-presentation-slice.md)。
