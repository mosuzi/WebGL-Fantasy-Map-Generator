...
正式互动地图的唯一 canonical owner 仍是 `createGeneratorApp` 内的 `state.map`。Facade 只接收 `getMap / getRevision / getHistoryFingerprint` 三个 getter；每次同步读取都重新取得 owner，既不缓存 map，也不允许 borrowed owner 引用直接返回或跨异步边界。
...
- commit envelope 的 projection 集合在后续更新中必须保持不变、不得重复，只有全部 `ready / degraded` 才能记为 `projections-settled`。
...
- async borrow、owner 逃逸、读取期换 owner、source revision 漂移、重复 projection、非法 lifecycle 均拒绝；
