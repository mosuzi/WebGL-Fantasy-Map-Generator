...
> 对应权威任务第 128、159、172 项。状态：调研、独立验证原型与沿岸对象保护门禁均已完成；不修改正式 renderer、地图几何或存档结构。
...
7. 海岸面积 P95 与双向 Hausdorff 只用于说明轮廓变化程度，不单独否决结果；海岸硬门禁改为拓扑与沿岸对象关系：城镇仍在原陆区、道路全线不穿岸、河流只在锚定河口入海。国界 / 省界的形状门槛继续用于硬验收。
...
4. 验证新弧线不与非相邻海岸相交、不改变 island / lake / ocean 嵌套；近岸城镇必须保持在原陆地区域，道路完整折线不得被海岸截断或进入水域，河流除末端外保持陆侧且河口严格复用指定海岸弧线端点锁。海岸面积 / Hausdorff 超限只记录诊断，不能替代这些对象关系门禁。
...
- 最大 Hausdorff / 点到线偏差按海岸、国界、省界分别记录；海岸超限只作形状诊断，国界 / 省界仍不得超过各层硬容差。
- 面积相对误差报告 P50 / P95 / max；海岸 P95 只作诊断，推荐管线的国家 P95 `≤ 0.5%`、省份 P95 `≤ 1%` 继续硬验收，小岛 / 小省另按绝对面积门槛保护。
- 狭窄海峡最小宽度、最小陆桥宽度和湖海连通分类不得改变。
- 近岸城镇保持在同一陆区，道路全线处于陆地且不与岸线相交，河流除河口外处于陆侧；河口必须等于声明的海岸 `ArcRef` 起点或终点，提前穿岸与漂移均为硬失败。
...
原型验证共享 arc、正反 `ArcRef`、锁定节点、不可变快照、fill / stroke 同源、环与 coverage 有效性、分层形状诊断和八类案例约束。单岛、狭窄海峡、湖海连接另带城镇、道路、河流 / 河口最小保护对象，定义阶段拒绝坏 region / arc、非有限坐标和非海岸河口，案例约束拒绝城镇越岸、道路入水、河口漂移及河流提前穿岸。海岸面积 / Hausdorff 超限只提示，行政边界形状门槛仍硬失败；默认算法矩阵为六种管线 `8/8`、B-spline `7/8`。它只验证数据结构和几何不变量，不代表 P0～P4 已进入正式渲染器。
