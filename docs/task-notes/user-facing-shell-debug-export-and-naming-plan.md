...
> 状态校准（2026-07-16）：用户外壳、开发模式、比例尺、控制面板、导入导出和中文命名主链已经落地。本文保存早期方案与证据，文中的“后续”不自动进入权威任务。
...
- 高度编辑面板中可以打开独立高度图导入工作台，在工作台里选择本地图片并调整最低/最高高度。
...
- 已实现灰度高度图导入第一刀，入口已从生成 tab 迁入高度编辑面板，并进一步收束到独立高度图导入工作台。
...
- 已补充导入前预览工作台：选择图片只更新 canvas 缩略预览和亮度统计，点击“应用到地图”后才触发 `grayscale-import` 完整重生成。
...
- 国家国号从“国/邦/王朝/盟/诸州”收敛为由政体、规模和自我定位决定的正式后缀；不再使用“朝”，小国多为 `国`，君主制可用 `王国 / 帝国`，共和或联邦政体可用 `共和国 / 联邦 / 联邦共和国`。
...
- 已将国家形制升级为政体驱动的国号规则，旧的“朝 / 侯国 / 伯国 / 自由邦”不再作为当前生成目标。
...
- 阶段 F 已作为第三批代码实现：春秋古国风国家根名、根族去重和形制收敛；后续又升级为政体驱动的国号规则。
- Element Plus 已作为第四批代码接入按需导入与 `UiButton` 样板迁移。
- 阶段 E 已作为第五批代码实现：灰度高度图导入、采样型高度模板、完整重生成链路和黑白反转映射；入口后续已迁入高度编辑面板，并在高度图导入工作台中完成导入前预览。
