...
- 支持用户选择本地灰度图。
- 读取像素亮度并映射到 grid cell 高度。
- 提供高度映射区间，例如黑色对应最低高度、白色对应最高高度，可调整海平面与峰值。
- 应用后重算 feature、climate、biome、pack、river、politics/settlements 等派生链路，或先明确第一刀只重算高度与基础水陆，再标记下游过期。
