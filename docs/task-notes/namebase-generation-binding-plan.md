...
本文档记录名称库从“可管理数据”进入“影响后续生成”的设计。当前名称库总览已支持导出、追加/替换导入、复制内置库、重命名、编辑用户库样本、删除和清空用户库；全局/文化绑定、整图生成继承、本地偏好、样本权重、Markov 链路和编辑历史第一刀均已接入。这些操作仍不自动改写当前地图对象名称。
