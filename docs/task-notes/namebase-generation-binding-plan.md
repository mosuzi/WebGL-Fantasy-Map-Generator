...
本文档记录名称库从“可管理数据”进入“影响后续生成”的设计。当前名称库总览已支持导出、追加/替换导入、复制内置库、重命名、编辑用户库样本、删除和清空用户库；全局/文化绑定、整图生成继承、本地偏好、样本权重、Markov 链路、编辑历史，国家/城市/河流筛选显式重命名，选中标签/河流/湖泊目标单个重命名，基础生成质量参数，以及原版文本导入导出第一刀均已接入。这些操作仍不自动改写当前地图对象名称。
...
- 已完成基础质量参数第一刀：绑定命中的用户库会读取 `minLength / maxLength / duplicateChars`，生成候选会按长度过滤，并拒绝非白名单字符相邻重复。
- 已完成原版文本导入导出第一刀：上传 `name|min|max|d|m|names` 文本会转换为用户库并保留长度与允许连写参数；`m` 字段只作为兼容元数据保存。
...
- 已完成国家显式重命名第一刀：国家管理面板可由用户主动把当前筛选结果按当前 `stateRoot` 名称库重新命名；该命令只写国家 `name/fullName`，保留已有政体国号后缀，不重建政区、外交、经济或军事，并避免未经空间校正的东南西北前缀兜底。
- 已完成选中对象显式重命名第一刀：对象详情面板在选中国家/城市标签时可按当前名称库只重命名对应目标；命令复用国家/城市已有显式重命名链路，进入 `EditHistory`，不自动改写其它地图对象。
...
- 生成预览已读取用户库基础质量参数，示例会按 `minLength / maxLength / duplicateChars` 过滤。
...
- 国家管理面板已完成第一刀：可对当前筛选结果执行显式重命名，读取全局/文化 `stateRoot` 绑定，并可撤销/重做。
- 对象详情面板已完成第二刀：选中国家/城市标签、河流或湖泊时，可只按名称库重命名对应目标，并可撤销/重做。
- 湖泊已建立对象类型、拾取、详情展示和单个名称库重命名链路；基础质量参数和原版文本兼容第一刀已完成，完整湖泊管理面板和更细质量诊断仍待后续扩展。
