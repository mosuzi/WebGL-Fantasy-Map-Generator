...
本文档记录名称库从“可管理数据”进入“影响后续生成”的设计。当前名称库总览已支持导出、追加/替换导入、复制内置库、重命名、编辑用户库样本、删除和清空用户库；但这些操作暂不改变地图对象名称，也不参与生成。本计划用于后续接入前统一边界。
...
- 在 `namebase-store.js` 增加绑定读写 helper。
- 名称库总览显示某个用户库是否被绑定使用。
- 绑定失效时能在面板中看到。
...
- 第一刀可以用加权抽样或现有组合规则，不急于复刻 Markov chain。
...
- 在文化管理面板或名称库面板中为文化设置 `stateRoot / place / hydro` 覆盖。
- 城市、河流和省份命名优先读取对象所属文化的绑定。

风险：

- 城市和国家共用同一词池会产生“清河国 / 清河城”式重复，应在 UI 上按 kind 过滤候选库。
...
- 可选实现纯函数 Markov chain，接近原版 `Names.calculateChain()`。

约束：

- Markov chain 必须是本项目自己的纯函数，不依赖 `source/` 全局状态。
- 示例生成不能改变地图 checksum。
