...
本文档记录名称库从“可管理数据”进入“影响后续生成”的设计。当前名称库总览已支持导出、追加/替换导入、复制内置库、重命名、编辑用户库样本、删除和清空用户库；全局/文化绑定、整图生成继承、本地偏好、样本权重、Markov 链路、编辑历史，国家、城市和河流筛选显式重命名，以及选中标签目标单个重命名第一刀均已接入。这些操作仍不自动改写当前地图对象名称。
...
- 湖泊已建立对象类型、拾取、详情展示和单个名称库重命名链路；完整湖泊管理面板和更细质量参数仍待后续扩展。
