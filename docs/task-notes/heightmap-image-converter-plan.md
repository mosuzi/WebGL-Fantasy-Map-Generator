...
> 状态校准（2026-07-16）：灰度 / 彩色色板、预览、量化、自动与手动赋高、差值热力图、profile 往返和待处理颜色审核均已完成当前封闭范围。3D 地形与 OBJ 继续作为远期候选，不在当前队列。

本文记录对照原版 Image Converter 后，WebGL 版高度图导入工作台的设计与完成证据。彩色图片识别已经通过可预览、可回退、可解释的独立工作台实现，没有塞入原灰度函数。
...
- 待处理颜色可以分页审核，每页显示 `12` 个；可以单个“加入”主色板，也可以把当前页显示的待处理色一次性“加入显示色”，并以各自当前自动高度作为显式高度；最终 `image-palette` 应用也会把这些显式加入的颜色桶纳入 assignments，即使它不在当前色板上限的前 N 个高频色里。
...
- 导入 profile 后工作台会显示配置色块匹配摘要，区分已匹配、未匹配和当前图片额外色块；失配时会列出未匹配的配置色和当前图片额外色，当前额外色可点击高亮预览区域，也可一键加入配置并写入手动 assignment；已纳入 profile 的当前色块可从配置移除并回到额外色统计；导出时会按当前 profile key 集保存配置，未加入的当前额外色不会进入 profile，未匹配但仍属于配置的色块会保留；支持先导入 profile 再选择图片，选择图片时不会丢失已导入的 profile assignments。
