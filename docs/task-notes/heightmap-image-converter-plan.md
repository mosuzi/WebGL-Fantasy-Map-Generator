...
- 色板已经可量化、自动估高和手动覆盖，但暂时只服务预览，不参与最终高度采样。
- 手动覆盖还没有批量选择、未分配颜色策略和 profile 导入导出。
- 点击应用仍走 `grayscale-import`，尚未生成 `kind = image-palette` 的 sampled heightmap。
...
- 当前阶段仍是预览能力，`应用到地图` 暂不消费这些 assignment。
...
- 手动覆盖会立即刷新色板显示和预览高亮，但只改工作台状态，不写 `map`。
- 下一步需要把当前色板、自动高度、手动覆盖和未分配策略整理为 `image-palette` 采样输入，再接入完整重生成和 JSON 元数据。
