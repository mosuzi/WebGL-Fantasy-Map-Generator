...
- 导入 profile 后工作台会显示配置色块匹配摘要，区分已匹配、未匹配和当前图片额外色块；失配时会列出未匹配的配置色和当前图片额外色，当前额外色可点击高亮预览区域，也可一键加入配置并写入手动 assignment；支持先导入 profile 再选择图片，选择图片时不会丢失已导入的 profile assignments。
