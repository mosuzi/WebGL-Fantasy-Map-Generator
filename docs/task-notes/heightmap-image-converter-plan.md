...
- 亮度直方图、采样格高度色带预览、应用前后对比和差值热力图已完成第一刀。
- 色板已经可量化、自动估高、手动覆盖并参与 `image-palette` 最终高度采样。
- 色板已经支持单色和批量手动覆盖；profile 导出和导入复用已完成第一刀。
- 未分配颜色策略已完成第一刀；`标记待处理` 已具备基础阻断、待处理颜色列表和单色转入主色板能力，后续仍可补审核队列或差值热力图。
...
- 工作台会基于当前预览 canvas 生成 24 桶亮度直方图，展示暗 / 中 / 亮像素占比。
- 工作台会把当前图片按导入设置映射成高度色带预览：默认灰度且无手动覆盖时走连续亮度高度，非灰度模式或手动覆盖时走当前色板 assignment 和未分配高度。
- 工作台会把当前地图高度摘要与导入预览高度摘要并排展示，用于应用前确认高度范围、水域占比、平均高度和水域变化；当前地图摘要只传小对象，不把完整高度数组交给 Vue。
- 工作台会基于当前地图的小型高度栅格绘制“差值热力图”，用橙色表示导入后升高、蓝色表示导入后降低、深色表示变化很小；该栅格约 `96` 像素宽，由 runtime 聚合后传给高度面板，不把完整 `grid.cells.h` 交给 Vue。
...
- 工作台新增“未分配颜色”策略，默认 `固定高度`；`合并最近色` 会把未进入色板上限的颜色按 RGB 距离归并到最近已分配色块；当导入会走 `image-palette` 且仍有未分配像素时，`标记待处理` 会阻止应用并提示用户扩大色板上限、改为合并最近色或切回固定高度。
- `标记待处理` 阻断时会列出未进入色板上限的前 `12` 个颜色桶，并提供“扩大色板”按钮把色板上限推进到下一档；该列表复用预览量化结果，不额外重扫图片。
- 待处理颜色可以单个“加入”主色板，并以当前自动高度作为显式高度；最终 `image-palette` 应用也会把这些显式加入的颜色桶纳入 assignments，即使它不在当前色板上限的前 N 个高频色里。
...
- 色板条目支持 checkbox 批量选择，可全选或清空；批量赋高面板可用滑条、预设按钮或“恢复自动”一次性修改多个色块。
