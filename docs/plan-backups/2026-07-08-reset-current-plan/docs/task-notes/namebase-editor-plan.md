...
本文档记录对照原版 `Namesbase Editor` 后，WebGL 版名称库能力的实现边界。当前生成器已有中文词池、春秋古国风国家根名和标签管理，也已具备用户名称库导入导出、编辑、全局绑定、文化级绑定、文化面板快捷入口、应用级本地偏好、样本权重、项目内 Markov 链路生成、编辑历史第一刀，国家/城市/河流/湖泊筛选显式重命名，选中标签/河流/湖泊目标单个重命名，基础生成质量参数、原版文本导入导出第一刀，以及更细质量诊断第一刀；后续重点是补更多对象面板入口和原版多词率 `m` 行为。
...
- 已补原版文本导入导出第一刀：可导出当前名称库为 `name|min|max|d|m|names` 文本，也可上传该格式并导入为用户名称库；`m` 暂只保留为兼容字段，不参与本项目生成。
- 已补更细质量诊断第一刀：名称库摘要会统计平均/中位样本长度、长度越界样本、相邻连写风险、特殊字符、加权样本数量和最高权重；面板详情和 JSON 导出同步展示，质量标签会优先提示需要校准的风险。

后续：

- 补更多对象面板入口和原版多词率 `m` 行为；湖泊管理面板级批量入口已完成。
...
1. 补更多对象面板的显式名称库入口，例如其它后续可命名对象。
2. 需要更接近原版时，再补多词率 `m` 行为和原版完整分析口径。
