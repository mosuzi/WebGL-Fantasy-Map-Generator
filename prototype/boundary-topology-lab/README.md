...
- 页面顶部“国界颜色过渡对照”是第 371 项的独立视觉试验：固定同一组锐角、凹角和三方交汇，比较第 370 项九轨、`v0.5.67` 历史单带、连续中心线色带与屏幕空间朦胧。宽度、强度和柔顺度只重绘这四张匿名画布，不改变下方拓扑夹具，也不写正式地图或偏好。
- “反向三角 / 宽度突变 / 三方交汇”用于解释肉眼尖片来源。历史单带只证明旧观感如何形成，不代表它已通过几何验收；屏幕空间朦胧没有偏移网格，因此显示“无网格 / 连续蒙版 / 圆角蒙版”。
...
- 独立国界颜色过渡夹具覆盖九轨失败基线、历史单带、先平滑后偏移和无偏移网格雾化；其诊断不计入既有 20 项拓扑分母。
