...
function rebuildPackSettlementsWithoutImplicitAnchors(grid, politics, pack, random, options, settlements, scope) {
  ...
  for (const state of politics?.states || []) {
    ...
    if (retainedCapital) {
      if (!protectedStateIds.has(stateId)) synchronizeStateCapital(politics, pack, retainedCapital);
      continue;
    }
  ...
  for (const province of politics?.provinces || []) {
    ...
    if (retainedProvincial) {
      if (!protectedProvinceIds.has(provinceId)) synchronizeProvinceCapital(politics, pack, retainedProvincial);
      continue;
    }
