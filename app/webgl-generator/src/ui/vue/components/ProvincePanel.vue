<template>
  ...
  <UiObjectTable
    ...
    resizable-columns
    selectable-rows
    :selected-row-ids="selectedProvinceIds"
    @select="callbacks.onSelect"
    @locate="callbacks.onLocate"
    @edit="openRenameEditor"
    @empty-action="handleEmptyAction"
    @column-resize="callbacks.onColumnResize"
    @selection-change="selectedProvinceIds = $event"
  />

  <UiPanelIoActions
    class-name="province-panel-list-actions"
    label="省份列表高亮"
    :actions="provinceHighlightActions"
...
const brushRadius = readBrushRadiusContract(BRUSH_RADIUS_ID.PROVINCE);
...
const regenerationLocks = useRegenerationLockSelection({panelId: "province-panel", kind: "province", rows: visibleRows});
const {selectedRowIds: selectedProvinceIds} = useVisibleRowSelection(visibleRows);
const selectedProvinceRows = regenerationLocks.selectedRows;
const highlightableProvinceRows = computed(() => selectedProvinceRows.value.filter(row => !row.neutral && Number(row.id) > 0));
...
const detailRows = computed(() => selected.value ? [
  ...
  {label: "城镇贡献（计入实力）", value: formatNumber(selected.value.settlementPower)},
...
function buildProvinceMetrics(map) {
  const rows = provinceRows(map).map(row => {
    ...
    return {
      ...
      settlementPower: neutral ? 0 : Number(province?.settlementPower || 0),
