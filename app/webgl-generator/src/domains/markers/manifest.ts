...
export const markersManifest = {
  ...
  commands: markerCommands.map(id => ({id, writeSet: id === "markers.setNote" ? ["notes"] : id === "markers.setVisual" ? ["markers"] : ["markers", "pack", "economy"], undoPolicy: "required", profiles: ["interactive", "headless"]})),
  ...
  queries: [
    {id: "markers.list", reads: ["markers"], profiles: ["interactive", "headless"]},
    {id: "markers.get", reads: ["markers", "notes"], profiles: ["interactive", "headless"]}
  ],
  ...
  api: {methods: [
    ...markerCommands.map(id => ({id: `edit.${id}`, method: `edit.${id}`, target: id, schema: `${id}.v1`, capability: "command" as const, errorCodes: [], documentation: `markers/${id}`})),
    {id: "markers.regenerationApi", method: "generate.regenerate", target: "markers.regenerateResources", schema: "generate.regenerate.markers.v1", capability: "regeneration", errorCodes: ["confirmation_required"], documentation: "markers/regeneration"}
  ]},
