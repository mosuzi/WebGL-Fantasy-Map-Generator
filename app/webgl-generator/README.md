...
当前标签仍是阶段性 overlay：尚未实现道路/国家标签、曲线文字、标签编辑和对象级点击。道路 mesh 目前还没有 dash、路线点击选择、道路等级样式和复杂急弯 bevel 策略；路线 hover picking 仍是直接遍历折线段的轻量实现。

## 面板规划

当前配置区和统计区仍是左侧固定面板。后续正式编辑体验需要统一改为 HTML 浮动可拖动面板，包含但不限于：
...
这些面板不使用 canvas 实现，应该作为普通 DOM UI 覆盖在地图工作区上方，并保留拖动、折叠、层级和可停靠/恢复位置的扩展空间。本条只是当前架构约束记录，暂不改动现有侧栏实现。
