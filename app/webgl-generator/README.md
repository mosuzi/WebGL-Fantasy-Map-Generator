...
当前河流河源上限会随 cells 动态变化，并降低固定 flux 阈值；路线已从贪心追踪改为 A* 成本寻路，陆路不会穿水，找不到路径时不会画终点直线。温度和降水视图会在画布显示图例，温度范围使用摄氏度单位。
...
- renderer 支持国家、省份和区域视图，水域保持独立底色。
- 悬停面板显示国家、省份和区域名称。
...
- UI 新增人口视图，运行时统计显示城市/首都/港口、道路/线段、人口点和点顶点。
...
- 政治对象已接入 selection fallback：未命中城市、路线和河流时，会按当前视图或默认省份逻辑选中国家、省份或区域，并刷新对象详情面板。
...
- 正式国家编辑器已完成第一刀：
  ...
  - 国家颜色变更同样进入 `EditHistory`，修改 `map.politics.states[*].color` 并刷新 states 视图颜色。
  - 国家编辑启用后自动切到 `states` 视图；拖动中按 `cell-colors` 语义刷新 cell 表面，抬手后刷新 selection/runtime/pick。
  - 当前是国家表层归属编辑，不重跑省份、区域、城市、路线、军事或 zones 派生系统。
- 正式编辑器交互锁已完成第一刀：
  - 高度编辑、国家编辑或对象编辑状态中，左侧生成、视图切换、视图操作和其它编辑入口会禁用。
...
这些面板不使用 canvas 实现，应该作为普通 DOM UI 覆盖在地图工作区上方，并保留拖动、折叠、层级和可停靠/恢复位置的扩展空间。当前已接入对象详情面板和独立河流管理面板；现有生成、视图和运行时统计侧栏暂不整体迁移。
