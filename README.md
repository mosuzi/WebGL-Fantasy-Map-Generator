...
- 轻量测量工具：可在地图上点击添加测量点，显示折线总长；三点及以上会按闭合多边形显示面积，并支持拖动测量点、插入测量点、删除测量点、撤销最后一点、一键清除和导出测量 JSON。
...
- 灰度高度图导入：读取本地灰度图，按亮度映射到用户指定高度区间，支持黑白反转、拉伸铺满或保持比例裁剪，并走完整地图派生重算链路。
- 开发模式：通过 `?debug=1` 或 `window.__webglGeneratorDebug.enabled = true` 打开，用于查看生成耗时、WebGL 统计、picking 统计和内部状态。

## 计划实现

- 组件库迁移：已接入 Element Plus 按需组件解析，并迁移按钮、输入框、数字输入、排序条、下拉、tabs、slider、switch、共享颜色选择器、对象表格、树状总览、图层开关、地图工具栏和测量读数等；后续进入弹窗等较重组件前，继续保持懒加载或拆包方案。
- 文件能力完善：图片导出继续合成图例、标签等 DOM overlay，完整地图数据压缩，GeoJSON 范围导出、国家/省份/区域 dissolve、导入格式兼容和错误诊断。
- 高度图导入增强：下一步按 `docs/task-notes/heightmap-image-converter-plan.md` 做导入预览工作台、轻量色板量化、亮度/色相/FMG 色带自动映射、平滑降噪和陆地比例预检。
- 更完整的导出格式：GeoJSON、后续可能的 SVG / 数据格式兼容。
- 更细的 source 语义对齐：贸易、军事、对象注记、zone、marker 和命名系统继续向原项目行为靠拢。
- 中文命名继续细化：当前已加入春秋古国风短名和单字国名策略，并对群舒、曾/鄫/缯、谭/郯等近源根名做家族去重；名称库总览已能查看、导出内置词池和质量提示，把名称库 JSON 追加导入或替换导入为随完整地图保存的用户库，也能新建用户库、复制内置库、重命名用户库、编辑用户库样本、生成样例预览、删除单个用户库或清空用户库；后续继续做权重、冲突预览、文化绑定、避讳规则和更稳定的省份/城市命名联动。
