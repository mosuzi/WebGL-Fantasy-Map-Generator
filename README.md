...
- 经济与贸易：经济总览可查看商品、市场、交易、运距成本和供需缺口，资源货物会影响适居度与静态路线权重；支持当前筛选结果 CSV/JSON 导出，地图可开启静态贸易流图层并查看贸易线详情。
- 军事战报链路：军事面板支持战斗事件记录、导入、筛选、清理、展开完整链路、轻量结果应用、战报摘要，以及按范围导出 CSV/JSON。
- 高度图导入工作台：支持灰度和彩色色板导入、自动映射、手动赋高、profile 导入导出、待处理颜色审核、分页和批量加入。
- 导出能力：支持图片导出、完整地图 JSON、部分 GeoJSON、名称库、政体 CSV/JSON、军事 CSV/JSON、战斗事件 CSV/JSON、外交 CSV/JSON 等本地文件。
...
- 经济贸易还需要价格传播、市场归属编辑、资源货物对城镇选址的进一步影响，以及更完整的贸易可视化。
