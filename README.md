...
- 名称库基础质量参数（生成长度、允许连写）和原版 `name|min|max|d|m|names` 文本导入导出第一刀已完成；后续主要补更多对象面板入口和更细质量诊断。导入、绑定、本地偏好、样例预览、权重、Markov 链路、撤销/重做、国家/城市/河流筛选显式重命名，以及选中标签、河流和湖泊目标单个重命名已经完成。
