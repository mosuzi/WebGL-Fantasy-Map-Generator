...
- 中文命名继续细化：当前已加入春秋古国风短名和单字国名策略，并对群舒、曾/鄫/缯、谭/郯等近源根名做家族去重；名称库总览已能查看、导出内置词池和质量提示，把名称库 JSON 追加导入或替换导入为随完整地图保存的用户库，也能复制内置库、重命名用户库、编辑用户库样本、删除单个用户库或清空用户库；后续继续做文化绑定、避讳规则和更稳定的省份/城市命名联动。
