...
- 对象管理面板：国家、政体、省份、城市、文化、宗教、河流、路线、资源标记、外交、军事、名称库和备注等都有独立浮动面板。
- 轻量编辑能力：可以调整部分对象名称、颜色、政体（含分组批量套用）、首都、城市剪影、资源点、外交关系、军事态势、驻地/基地、兵种比例和高度编辑笔刷；国家、城市和河流可按当前名称库对筛选结果执行显式重命名，选中的国家/城市标签、河流和湖泊也可只重命名对应目标并撤销。
...
- 名称库基础质量参数（生成长度、允许连写）、更细质量诊断和原版 `name|min|max|d|m|names` 文本导入导出第一刀已完成；后续主要补更多对象面板入口和原版多词率 `m` 行为。导入、绑定、本地偏好、样例预览、权重、Markov 链路、撤销/重做、国家/城市/河流筛选显式重命名，以及选中标签、河流和湖泊目标单个重命名已经完成。
